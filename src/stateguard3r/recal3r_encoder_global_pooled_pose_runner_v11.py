"""Independent pinned recurrent loop for v11 encoder-global pose export."""
...
from .encoder_global_pooled_pose_v11 import EncoderGlobalPoseCapture, EncoderGlobalPooledPoseError, capture_encoder_global_pose_token
...
class EncoderGlobalPooledPoseRunnerError(RuntimeError):
    """Raised when v11's isolated recurrent execution is unsafe."""


class EncoderGlobalPooledPoseObserver(Protocol):
    def observe(self, frame_id: int, prediction: Mapping[str, Any], model: Any, frame_context: Any, encoder_global_pose_capture: EncoderGlobalPoseCapture) -> Any: ...
...
def audit_v11_runner_contract(runner_path: Path) -> Mapping[str, Any]:
    """Fail closed unless v11 remains an encoder-global pre-retrieval capture."""
    source = runner_path.read_text(encoding="utf-8")
    try:
        body = source[source.index("\ndef run_encoder_global_pooled_pose_recurrent_lighter(") + 1:]
    except ValueError as error:
        raise EncoderGlobalPooledPoseRunnerError("v11 runner entry point is missing") from error
    global_feature = "global_img_feat_i = model._get_img_level_feat(feat_i)"
    ...
    try:
        global_at = body.index(global_feature)
        capture_at = body.index(capture, global_at)
        pose_branch_at = body.index("if model.pose_head_flag:\n            pose_feat_i", capture_at)
        inquire_at = body.index(inquire, capture_at)
        ...
        candidate_at = body.index("else:\n            if encoder_global_pose_capture is None:", control_at)
        ...
        raise EncoderGlobalPooledPoseRunnerError("v11 runner source contract changed") from error
    if not (global_at < capture_at < pose_branch_at < inquire_at < rollout_at < update_at):
        raise EncoderGlobalPooledPoseRunnerError("v11 encoder-global capture is not before retrieval, rollout and memory update")
    if not (candidate_at < finalize_at < candidate_transfer_at) or control_transfer_at < control_at:
        raise EncoderGlobalPooledPoseRunnerError("v11 export/control transfer order changed")
    capture_region = body[global_at:pose_branch_at]
    forbidden_capture = ("dec[", "model.pose_token", "camera_pose", "prediction", ".cpu(", ".numpy(", ".tolist(", "ground_truth", "future", "anchor", "history", "fallback")
    if any(marker in capture_region for marker in forbidden_capture):
        raise EncoderGlobalPooledPoseRunnerError("v11 capture boundary contains forbidden data")
    control = body[control_at:candidate_at]
    if any(marker in control for marker in ("capture_encoder_global_pose_token(", "decoder_embed(", ".detach().clone()", "observer.")):
        raise EncoderGlobalPooledPoseRunnerError("v11 always-control path is contaminated by candidate policy work")
    ...
    if any(marker in body for marker in previous_recovery_markers):
        raise EncoderGlobalPooledPoseRunnerError("v11 runner delegates to a previous recovery mechanism")
    return {
        ...
        "encoder_spatial_token_count_is_runtime_evidence": True,
        "aggregation": "native_model._get_img_level_feat",
        "frozen_decoder_projection": "model.decoder_embed",
        "capture_relation": "after_feature_selection_before_pose_retriever_rollout_update_mem",
...
class EncoderGlobalPooledPoseRunnerResult:
...
def run_encoder_global_pooled_pose_recurrent_lighter(
    ...
    observer: EncoderGlobalPooledPoseObserver | None = None, watchdog_limit: int = 8, verify_source: bool = True,
    synchronize: Callable[[], None] | None = None,
) -> EncoderGlobalPooledPoseRunnerResult:
    """Run the pinned lighter body with an isolated v11 pre-retrieval hook."""
    if not views:
        raise EncoderGlobalPooledPoseRunnerError("views must not be empty")
    ...
    for i, raw_view in enumerate(views):
        if i and _reset_requested(raw_view.get("reset")):
            raise EncoderGlobalPooledPoseRunnerError("v11 recovery fails closed at a non-initial reset boundary")
        ...
        if len(loaded_image_shape) != 2 or any(item <= 0 for item in loaded_image_shape):
            raise EncoderGlobalPooledPoseRunnerError("v11 loaded image shape is unavailable")
        ...
        if img_out is not None and ray_out is None:
            ...
            raise EncoderGlobalPooledPoseRunnerError("ReCal3R frame has neither image nor raymap features")
        if i == 0:
        ...
        encoder_global_pose_capture = None
        if observer is not None:
            if not model.pose_head_flag:
                raise EncoderGlobalPooledPoseRunnerError("v11 requires the pinned pose-head path")
            try:
                encoder_global_pose_capture = capture_encoder_global_pose_token(
                    global_img_feat_i, encoder_feature_shape=tuple(feat_i.shape), projection=model.decoder_embed, torch=torch,
                )
            except EncoderGlobalPooledPoseError as error:
                raise EncoderGlobalPooledPoseRunnerError(f"v11 encoder-global capture failed closed: {error}") from error
        ...
        if len(dec) != model.dec_depth + 1:
            raise EncoderGlobalPooledPoseRunnerError("pinned ReCal3R decoder depth changed")
        ...
        if observer is None:
            ...
            if encoder_global_pose_capture is None:
                raise EncoderGlobalPooledPoseRunnerError("v11 candidate lacks encoder-global capture")
            if frame_context is None:
                candidate_context: Mapping[str, Any] = {"v11_loaded_image_shape": list(loaded_image_shape)}
            elif isinstance(frame_context, Mapping):
                candidate_context = {**frame_context, "v11_loaded_image_shape": list(loaded_image_shape)}
            else:
                raise EncoderGlobalPooledPoseRunnerError("v11 frame context must be a mapping")
            observation = observer.observe(i, res, model, candidate_context, encoder_global_pose_capture)
            ...
            if quarantine:
                if pre is None or witness is None:
                    raise EncoderGlobalPooledPoseRunnerError("v11 quarantine has no pre-state witness")
                try:
                    ...
                    raise EncoderGlobalPooledPoseRunnerError(f"v11 restore failed: {error}") from error
    ...
    return EncoderGlobalPooledPoseRunnerResult(predictions, views, timeline, source_provenance, elapsed)


__all__ = [
    "EncoderGlobalPooledPoseObserver", "EncoderGlobalPooledPoseRunnerError", "EncoderGlobalPooledPoseRunnerResult",
    "audit_v11_runner_contract", "run_encoder_global_pooled_pose_recurrent_lighter",
]
