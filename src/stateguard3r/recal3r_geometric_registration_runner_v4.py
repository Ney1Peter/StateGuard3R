"""Independent pinned recurrent loop for v4 geometric registration export."""
...
class GeometricRegistrationRunnerError(RuntimeError):
    """Raised when v4's isolated recurrent execution is unsafe."""


class GeometricRegistrationObserver(Protocol):
...
class GeometricRegistrationRunnerResult:
...
def run_geometric_registration_recurrent_lighter(
    ...
    observer: GeometricRegistrationObserver | None = None, watchdog_limit: int = 8, verify_source: bool = True,
    synchronize: Callable[[], None] | None = None,
) -> GeometricRegistrationRunnerResult:
    """Run the pinned lighter body with v4 rollback and geometric-only export hooks."""
    if not views:
        raise GeometricRegistrationRunnerError("views must not be empty")
    ...
    for i, raw_view in enumerate(views):
        if i and _reset_requested(raw_view.get("reset")):
            raise GeometricRegistrationRunnerError("v4 recovery fails closed at a non-initial reset boundary")
        ...
        # on this boundary so policy/recurrent runtime remains comparable.
        ...
        if img_out is not None and ray_out is None:
            ...
            raise GeometricRegistrationRunnerError("ReCal3R frame has neither image nor raymap features")
        if i == 0:
        ...
        if len(dec) != model.dec_depth + 1:
            raise GeometricRegistrationRunnerError("pinned ReCal3R decoder depth changed")
        ...
        if quarantine:
            if pre is None or witness is None:
                raise GeometricRegistrationRunnerError("v4 quarantine has no pre-state witness")
            try:
                ...
                raise GeometricRegistrationRunnerError(f"v4 restore failed: {error}") from error
    ...
    return GeometricRegistrationRunnerResult(predictions, views, timeline, source_provenance, elapsed)


__all__ = ["GeometricRegistrationObserver", "GeometricRegistrationRunnerError", "GeometricRegistrationRunnerResult", "run_geometric_registration_recurrent_lighter"]
