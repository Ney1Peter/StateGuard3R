"""Independent pinned recurrent loop for v6 pre-rollout pose-query export."""
...
class PreRolloutPoseQueryRunnerError(RuntimeError):
    """Raised when v6's isolated recurrent execution is unsafe."""


class PreRolloutPoseQueryObserver(Protocol):
    def observe(self, frame_id: int, prediction: Mapping[str, Any], model: Any, frame_context: Any, pre_rollout_pose_query: Any) -> Any: ...
...
class PreRolloutPoseQueryRunnerResult:
...
def run_prerollout_pose_query_recurrent_lighter(
    ...
    observer: PreRolloutPoseQueryObserver | None = None, watchdog_limit: int = 8, verify_source: bool = True,
    synchronize: Callable[[], None] | None = None,
) -> PreRolloutPoseQueryRunnerResult:
    """Run the pinned lighter body with v6 rollback and query-export hooks."""
    if not views:
        raise PreRolloutPoseQueryRunnerError("views must not be empty")
    ...
    for i, raw_view in enumerate(views):
        if i and _reset_requested(raw_view.get("reset")):
            raise PreRolloutPoseQueryRunnerError("v6 recovery fails closed at a non-initial reset boundary")
        ...
        # here so the v6 recurrent/policy interval remains comparable.
        ...
        if img_out is not None and ray_out is None:
            ...
            raise PreRolloutPoseQueryRunnerError("ReCal3R frame has neither image nor raymap features")
        if i == 0:
        ...
        # This clone is intentionally captured before recurrent rollout and
        # before update_mem.  It is an external temporary export input only.
        pre_rollout_pose_query = pose_feat_i.detach().clone() if pose_feat_i is not None else None
        ...
        if len(dec) != model.dec_depth + 1:
            raise PreRolloutPoseQueryRunnerError("pinned ReCal3R decoder depth changed")
        ...
        observation = observer.observe(i, res, model, frame_context, pre_rollout_pose_query) if observer is not None else None
        ...
        if quarantine:
            if pre is None or witness is None:
                raise PreRolloutPoseQueryRunnerError("v6 quarantine has no pre-state witness")
            try:
                ...
                raise PreRolloutPoseQueryRunnerError(f"v6 restore failed: {error}") from error
        ...
        if observer is None:
            ...
            # Query decoding and pose replacement occur before the normal
            # prediction transfer; no policy input is materialized on CPU.
    ...
    return PreRolloutPoseQueryRunnerResult(predictions, views, timeline, source_provenance, elapsed)


__all__ = ["PreRolloutPoseQueryObserver", "PreRolloutPoseQueryRunnerError", "PreRolloutPoseQueryRunnerResult", "run_prerollout_pose_query_recurrent_lighter"]
