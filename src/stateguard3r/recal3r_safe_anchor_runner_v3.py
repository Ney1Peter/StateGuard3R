"""Independent pinned recurrent loop for v3 safe-anchor export recovery."""
...
class SafeAnchorRunnerError(RuntimeError):
    """Raised when v3's isolated recurrent execution is unsafe."""


class SafeAnchorObserver(Protocol):
...
def run_safe_anchor_recurrent_lighter(
    ...
    if not views:
        raise SafeAnchorRunnerError("views must not be empty")
    ...
    for i, raw_view in enumerate(views):
        ...
        if img_out is not None and ray_out is None:
            ...
            raise SafeAnchorRunnerError("ReCal3R frame has neither image nor raymap features")
        if i == 0:
        ...
        if len(dec) != model.dec_depth + 1:
            raise SafeAnchorRunnerError("pinned ReCal3R decoder depth changed")
        ...
        if quarantine:
            if pre is None or witness is None:
                raise SafeAnchorRunnerError("safe-anchor quarantine has no pre-state witness")
            try:
                ...
                raise SafeAnchorRunnerError(f"safe-anchor restore failed: {error}") from error
...
__all__ = ["SafeAnchorObserver", "SafeAnchorRunnerError", "SafeAnchorRunnerResult", "run_safe_anchor_recurrent_lighter"]
