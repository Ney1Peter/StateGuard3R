"""Independent pinned recurrent loop for v12 patch-embedding pose export."""
...
class PatchEmbedGlobalPooledPoseRunnerError(RuntimeError):
    """Raised when v12's isolated recurrent execution is unsafe."""


class PatchEmbedGlobalPooledPoseObserver(Protocol):
    def observe(self, frame_id: int, prediction: Mapping[str, Any], model: Any, frame_context: Any, patch_embed_global_pose_capture: PatchEmbedGlobalPoseCapture) -> Any: ...
...
def _v12_frame_context(frame_context: Any, loaded_image_shape: tuple[int, int]) -> Mapping[str, Any]:
    """Pass only causal scalar overlap plus image geometry to the observer.

    The current RGB tensor is necessarily consumed by the native patch embedder,
    but must not cross the candidate metadata boundary.
    """
    if frame_context is None:
        overlap = None
    elif isinstance(frame_context, Mapping):
        overlap = frame_context.get("overlap")
    else:
        raise PatchEmbedGlobalPooledPoseRunnerError("v12 frame context must be a mapping")
    return {"overlap": overlap, "v12_loaded_image_shape": list(loaded_image_shape)}


def audit_v12_runner_contract(runner_path: Path) -> Mapping[str, Any]:
    """Fail closed unless v12 keeps one candidate-only pre-RoPE patch capture."""
    source = runner_path.read_text(encoding="utf-8")
    try:
        body = source[source.index("\ndef run_patch_embed_global_pooled_pose_recurrent_lighter(") + 1:]
    except ValueError as error:
        raise PatchEmbedGlobalPooledPoseRunnerError("v12 runner entry point is missing") from error
    patch_call = "patch_tokens_i, _patch_pos_i = model.patch_embed(selected_imgs, true_shape=selected_shapes)"
    capture = "patch_embed_global_pose_capture = capture_patch_embed_global_pose_token("
    encode = "model._encode_image(selected_imgs, selected_shapes)"
    inquire = "model.pose_retriever.inquire(global_img_feat_i, mem)"
    ...
    try:
        patch_at = body.index(patch_call)
        capture_at = body.index(capture, patch_at)
        encode_at = body.index(encode, capture_at)
        inquire_at = body.index(inquire, encode_at)
        rollout_at = body.index(rollout, inquire_at)
        update_at = body.index(update, rollout_at)
        control_at = body.index("if observer is None:", update_at)
        candidate_at = body.index("else:\n            if patch_embed_global_pose_capture is None:", control_at)
        ...
        raise PatchEmbedGlobalPooledPoseRunnerError("v12 runner source contract changed") from error
    if body.count(patch_call) != 1 or body.count(capture) != 1:
        raise PatchEmbedGlobalPooledPoseRunnerError("v12 runner must contain exactly one candidate patch capture")
    if not (patch_at < capture_at < encode_at < inquire_at < rollout_at < update_at):
        raise PatchEmbedGlobalPooledPoseRunnerError("v12 patch capture is not before encoder, retrieval, rollout and memory update")
    if not (candidate_at < finalize_at < candidate_transfer_at) or control_transfer_at < control_at:
        raise PatchEmbedGlobalPooledPoseRunnerError("v12 export/control transfer order changed")
    capture_region = body[patch_at:encode_at]
    forbidden_capture = (
        "dec[", "model.pose_token", "pose_retriever.inquire", "camera_pose", "prediction", ".cpu(", ".numpy(", ".tolist(",
        "ground_truth", "future", "anchor", "history", "fallback",
    )
    if any(marker in capture_region for marker in forbidden_capture):
        raise PatchEmbedGlobalPooledPoseRunnerError("v12 patch capture boundary contains forbidden data")
    control = body[control_at:candidate_at]
    if any(marker in control for marker in ("model.patch_embed(", "capture_patch_embed_global_pose_token(", "decoder_embed", ".detach().clone()", "observer.")):
        raise PatchEmbedGlobalPooledPoseRunnerError("v12 always-control path is contaminated by candidate policy work")
    if "return {\"overlap\": overlap, \"v12_loaded_image_shape\": list(loaded_image_shape)}" not in source:
        raise PatchEmbedGlobalPooledPoseRunnerError("v12 capture metadata boundary changed")
    previous_recovery_markers = (
        "early_decoder_pose_runner_v8", "early_spatial_pooled_pose_runner_v9", "safe_anchor_runner_v3",
        "geometric_registration_runner_v4", "current_pointmap_runner_v5", "prerollout_pose_query_runner_v6",
        "aspect_ratio_spatial_grid_pose_runner_v10", "encoder_global_pooled_pose_runner_v11",
        "anchor_export_v3", "registration_export_v4", "pointmap_consensus_export_v5", "pose_query_export_v6",
    )
    if any(marker in body for marker in previous_recovery_markers):
        raise PatchEmbedGlobalPooledPoseRunnerError("v12 runner delegates to a previous recovery mechanism")
    return {
        "runner_path": str(runner_path.resolve(strict=True)),
        "runner_sha256": hashlib.sha256(source.encode("utf-8")).hexdigest(),
        "patch_feature_shape": [1, "N", 1024],
        "patch_global_shape": [1, 1, 1024],
        "projected_shape": [1, 1, 768],
        "actual_patch_token_count_is_runtime_evidence": True,
        "aggregation": "torch.mean(dim=1, keepdim=True) in isolated v12 primitive",
        "frozen_decoder_projection": "model.decoder_embed",
        "capture_relation": "after_feature_selection_before_full_encoder_retrieval_rollout_update_mem",
        "candidate_only_patch_embed_projection_and_clone": True,
        "candidate_metadata": "causal_overlap_and_loaded_height_width_only",
        "always_control_uses_legacy_raw_transfer": True,
        "candidate_finalize_after_rollback_before_cpu_transfer": True,
        "previous_recovery_delegation": False,
    }


@dataclass(frozen=True)
class PatchEmbedGlobalPooledPoseRunnerResult:
...
def run_patch_embed_global_pooled_pose_recurrent_lighter(
    ...
    observer: PatchEmbedGlobalPooledPoseObserver | None = None, watchdog_limit: int = 8, verify_source: bool = True,
    synchronize: Callable[[], None] | None = None,
) -> PatchEmbedGlobalPooledPoseRunnerResult:
    """Run the pinned lighter body with an isolated v12 pre-RoPE patch hook."""
    if not views:
        raise PatchEmbedGlobalPooledPoseRunnerError("views must not be empty")
    ...
    for i, raw_view in enumerate(views):
        if i and _reset_requested(raw_view.get("reset")):
            raise PatchEmbedGlobalPooledPoseRunnerError("v12 recovery fails closed at a non-initial reset boundary")
        ...
        loaded_image_shape = tuple(int(item) for item in view["img"].shape[-2:])
        if len(loaded_image_shape) != 2 or any(item <= 0 for item in loaded_image_shape):
            raise PatchEmbedGlobalPooledPoseRunnerError("v12 loaded image shape is unavailable")
        ...
        patch_embed_global_pose_capture = None
        if observer is not None:
            if not model.pose_head_flag:
                raise PatchEmbedGlobalPooledPoseRunnerError("v12 requires the pinned pose-head path")
            if not selected_imgs.size(0):
                raise PatchEmbedGlobalPooledPoseRunnerError("v12 current frame has no image patch feature")
            try:
                patch_tokens_i, _patch_pos_i = model.patch_embed(selected_imgs, true_shape=selected_shapes)
                patch_embed_global_pose_capture = capture_patch_embed_global_pose_token(
                    patch_tokens_i, projection=model.decoder_embed, torch=torch,
                )
            except PatchEmbedGlobalPooledPoseError as error:
                raise PatchEmbedGlobalPooledPoseRunnerError(f"v12 patch-embed capture failed closed: {error}") from error
        ...
        if img_out is not None and ray_out is None:
            ...
            raise PatchEmbedGlobalPooledPoseRunnerError("ReCal3R frame has neither image nor raymap features")
        if i == 0:
        ...
        if len(dec) != model.dec_depth + 1:
            raise PatchEmbedGlobalPooledPoseRunnerError("pinned ReCal3R decoder depth changed")
        ...
        if observer is None:
            ...
            if patch_embed_global_pose_capture is None:
                raise PatchEmbedGlobalPooledPoseRunnerError("v12 candidate lacks patch-embed capture")
            candidate_context = _v12_frame_context(frame_context, loaded_image_shape)
            observation = observer.observe(i, res, model, candidate_context, patch_embed_global_pose_capture)
            ...
            if quarantine:
                if pre is None or witness is None:
                    raise PatchEmbedGlobalPooledPoseRunnerError("v12 quarantine has no pre-state witness")
                try:
                    ...
                    raise PatchEmbedGlobalPooledPoseRunnerError(f"v12 restore failed: {error}") from error
    ...
    return PatchEmbedGlobalPooledPoseRunnerResult(predictions, views, timeline, source_provenance, elapsed)


__all__ = [
    "PatchEmbedGlobalPooledPoseObserver", "PatchEmbedGlobalPooledPoseRunnerError", "PatchEmbedGlobalPooledPoseRunnerResult",
    "audit_v12_runner_contract", "run_patch_embed_global_pooled_pose_recurrent_lighter",
]
