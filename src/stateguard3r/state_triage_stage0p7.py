"""Frozen methods for the independent 80-window Stage 0.7 benchmark."""
...
METHODS = ("S0.7", "C0.7", "T0.7")
DECISION_CONTRACT: dict[str, Any] = {
    "typed_method": "T0.7",
    "coverage_ablation_method": "C0.7",
    "scalar_method": "S0.7",
...
class Stage0p7DecisionError(Stage0p5DecisionError):
    pass


def _finite(row: Mapping[str, Any], name: str, *, allow_none: bool = False) -> float | None:
    if name not in row:
        raise Stage0p7DecisionError(f"evidence row lacks {name}")
    value = row[name]
    if value is None and allow_none:
        return None
    if isinstance(value, bool) or not isinstance(value, (int, float)) or not math.isfinite(float(value)):
        raise Stage0p7DecisionError(f"evidence row {name} must be finite")
    return float(value)
...
def decide_frame(method: str, row: Mapping[str, Any]) -> FrameDecision:
    if method not in METHODS:
        raise Stage0p7DecisionError("unsupported method")
    ...
    if method == "T0.7" and clipped >= float(DECISION_CONTRACT["clipped_fraction_transient_min"]):
        return _decision("transient_local_content", "local_clipped_content_before_coverage")
    if coverage < float(DECISION_CONTRACT["coverage_novelty_max_exclusive"]):
        return _decision("normal_novelty", "coverage_below_predeclared_novelty_threshold")
    if method == "C0.7" and clipped >= float(DECISION_CONTRACT["clipped_fraction_transient_min"]):
...
def decide_event(rows: Sequence[Mapping[str, Any]], *, method: str, start_frame: int, end_frame: int) -> dict[str, Any]:
    if start_frame < 0 or end_frame < start_frame or end_frame >= len(rows):
        raise Stage0p7DecisionError("event frame range is invalid")
...
__all__ = ["CAUSES", "DECISION_CONTRACT", "INTERACTION_CONTRACT", "METHODS", "UNRESOLVED", "Stage0p7DecisionError", "cause_metrics", "decide_event", "decide_frame"]
