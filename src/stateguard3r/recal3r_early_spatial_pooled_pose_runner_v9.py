"""Independent pinned recurrent loop for v9 spatial-pooled pose export."""
...
class EarlySpatialPooledPoseRunnerError(RuntimeError):
    """Raised when v9's isolated recurrent execution is unsafe."""


class EarlySpatialPooledPoseObserver(Protocol):
    def observe(self, frame_id: int, prediction: Mapping[str, Any], model: Any, frame_context: Any, early_spatial_pooled_pose_token: Any) -> Any: ...
...
def audit_v9_runner_contract(runner_path: Path) -> Mapping[str, Any]:
    """Fail closed unless v9 retains its exact candidate-only spatial mean boundary."""
    source = runner_path.read_text(encoding="utf-8")
    try:
        body = source[source.index("\ndef run_early_spatial_pooled_pose_recurrent_lighter(") + 1:]
    except ValueError as error:
        raise EarlySpatialPooledPoseRunnerError("v9 runner entry point is missing") from error
    ...
    try:
        ...
        guard_at = body.index('tuple(getattr(dec[0], "shape", ())) != (1, 576, 1024)', rollout_at)
        ...
        raise EarlySpatialPooledPoseRunnerError("v9 runner source contract changed") from error
    if not (rollout_at < guard_at < capture_at < update_at):
        raise EarlySpatialPooledPoseRunnerError("v9 spatial capture is not after rollout before memory update")
    if not (candidate_at < finalize_at < candidate_transfer_at) or control_transfer_at < control_at:
        raise EarlySpatialPooledPoseRunnerError("v9 export/control transfer order changed")
    control = body[control_at:candidate_at]
    if any(marker in control for marker in ("early_spatial_pooled_pose_token =", "decoder_embed(", ".mean(", "observer.")):
        raise EarlySpatialPooledPoseRunnerError("v9 always-control path is contaminated by candidate policy work")
    ...
    if any(marker in body for marker in previous_recovery_markers):
        raise EarlySpatialPooledPoseRunnerError("v9 runner delegates to a previous recovery mechanism")
    return {
...
class EarlySpatialPooledPoseRunnerResult:
...
def run_early_spatial_pooled_pose_recurrent_lighter(
    ...
    observer: EarlySpatialPooledPoseObserver | None = None, watchdog_limit: int = 8, verify_source: bool = True,
    synchronize: Callable[[], None] | None = None,
) -> EarlySpatialPooledPoseRunnerResult:
    """Run the pinned lighter body with an isolated v9 post-rollout export hook."""
    if not views:
        raise EarlySpatialPooledPoseRunnerError("views must not be empty")
    ...
    for i, raw_view in enumerate(views):
        if i and _reset_requested(raw_view.get("reset")):
            raise EarlySpatialPooledPoseRunnerError("v9 recovery fails closed at a non-initial reset boundary")
        ...
        if img_out is not None and ray_out is None:
            ...
            raise EarlySpatialPooledPoseRunnerError("ReCal3R frame has neither image nor raymap features")
        if i == 0:
        ...
        if len(dec) != model.dec_depth + 1:
            raise EarlySpatialPooledPoseRunnerError("pinned ReCal3R decoder depth changed")
        if not bool(torch.is_tensor(dec[0])) or tuple(getattr(dec[0], "shape", ())) != (1, 576, 1024):
            raise EarlySpatialPooledPoseRunnerError("pinned early spatial sequence shape changed")
        if not bool(torch.is_floating_point(dec[0])) or not bool(torch.isfinite(dec[0]).all()):
            raise EarlySpatialPooledPoseRunnerError("pinned early spatial sequence is nonfinite")
        # Candidate only: pool every early pre-projection spatial token, apply
        # the frozen official 1024-to-768 projection once, then isolate it.
        early_spatial_pooled_pose_token = None
        if observer is not None:
            ...
            if type(projection) is not torch.nn.Linear or projection.in_features != 1024 or projection.out_features != 768:
                raise EarlySpatialPooledPoseRunnerError("pinned frozen decoder projection changed")
            if projection.weight.device != dec[0].device or projection.weight.dtype != dec[0].dtype:
                raise EarlySpatialPooledPoseRunnerError("pinned frozen decoder projection device or dtype changed")
            try:
                early_spatial_pooled_pose_token = model.decoder_embed(dec[0].mean(dim=1, keepdim=True)).detach().clone()
            except Exception as error:
                raise EarlySpatialPooledPoseRunnerError(f"pinned early spatial projection failed: {error}") from error
            if not bool(torch.is_tensor(early_spatial_pooled_pose_token)) or tuple(getattr(early_spatial_pooled_pose_token, "shape", ())) != (1, 1, 768) or not bool(torch.is_floating_point(early_spatial_pooled_pose_token)) or not bool(torch.isfinite(early_spatial_pooled_pose_token).all()):
                raise EarlySpatialPooledPoseRunnerError("pinned early spatial pooled token is unavailable")
        ...
        if observer is None:
            ...
            observation = observer.observe(i, res, model, frame_context, early_spatial_pooled_pose_token)
            ...
            if quarantine:
                if pre is None or witness is None:
                    raise EarlySpatialPooledPoseRunnerError("v9 quarantine has no pre-state witness")
                try:
                    ...
                    raise EarlySpatialPooledPoseRunnerError(f"v9 restore failed: {error}") from error
    ...
    return EarlySpatialPooledPoseRunnerResult(predictions, views, timeline, source_provenance, elapsed)


__all__ = ["EarlySpatialPooledPoseObserver", "EarlySpatialPooledPoseRunnerError", "EarlySpatialPooledPoseRunnerResult", "audit_v9_runner_contract", "run_early_spatial_pooled_pose_recurrent_lighter"]
