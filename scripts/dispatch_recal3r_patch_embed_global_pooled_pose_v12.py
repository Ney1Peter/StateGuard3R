#!/usr/bin/env python3
"""One-use terminal-evidence dispatcher for the distinct v12 GPU route.

This is intentionally a v12-owned implementation: it does not import or call
...
RUNNER_RELATIVE = Path("scripts/run_recal3r_patch_embed_global_pooled_pose_export_v12.py")
PINNED_COMPONENTS = (
    RUNNER_RELATIVE,
    Path("src/stateguard3r/recal3r_patch_embed_global_pooled_pose_runner_v12.py"),
...
CONTROL_RUN_ID = "recovery-patch-embed-global-pooled-pose-v12-dynamic-always-commit-0001"
CANDIDATE_RUN_ID = "recovery-patch-embed-global-pooled-pose-v12-dynamic-candidate-0001"
CANDIDATE_POLICY = "detector-v3-incremental-patch-embed-global-pooled-pose-export"
RUN_SPECS = {
...
class V12DispatchError(RuntimeError):
    """No valid single-owner v12 dispatch/evidence path exists."""


class V12PreexecNoGo(V12DispatchError):
...
def _run_id(run_id: str) -> None:
    ...
    if type(run_id) is not str or not run_id.startswith("recovery-patch-embed-global-pooled-pose-v12-") or not run_id or any(ch not in valid for ch in run_id):
        raise V12DispatchError("run id is not a canonical v12 one-use id")
...
def _direct_directory(path: Path, label: str) -> None:
    if _lstat_kind(path) != "directory":
        raise V12DispatchError(f"{label} parent must be an existing non-symlink directory")
...
def _canonical_paths(paths: ArtifactPaths) -> None:
    ...
    for name, wanted in expected.items():
        ...
        if actual != wanted or actual.parent != wanted.parent:
            raise V12DispatchError(f"{name} path is not a direct canonical child")
...
def _read_regular_nul_free(path: Path, *, label: str) -> bytes:
    if _lstat_kind(path) != "regular":
        raise V12DispatchError(f"{label} is missing, a symlink, or nonregular: {path}")
    data = path.read_bytes()
    if b"\0" in data:
        raise V12DispatchError(f"{label} contains NUL: {path}")
    return data


def _write(path: Path, data: str, mode: int = 0o644) -> None:
    if "\0" in data or _lexists(path):
        raise V12DispatchError(f"refusing to overwrite or NUL-write one-use artifact: {path}")
...
def _freeze(path: Path) -> None:
    ...
    if stat.S_IMODE(path.stat().st_mode) != 0o444:
        raise V12DispatchError(f"could not freeze terminal artifact: {path}")
...
def _output_inventory(path: Path, *, frozen: bool) -> list[dict[str, Any]]:
    if _lstat_kind(path) != "directory":
        raise V12DispatchError("child did not produce a direct regular output directory")
    if frozen and stat.S_IMODE(path.stat().st_mode) != 0o555:
        raise V12DispatchError("output root mode is not frozen 0555")
    ...
    for current_text, directories, files in os.walk(path, topdown=False, followlinks=False):
        ...
        for name in sorted(files):
            ...
            if _lstat_kind(child) != "regular":
                raise V12DispatchError(f"output contains symlink or special artifact: {child}")
            _read_regular_nul_free(child, label="output artifact")
            if frozen and stat.S_IMODE(child.stat().st_mode) != 0o444:
                raise V12DispatchError(f"output artifact mode is not frozen 0444: {child}")
        ...
        for name in sorted(directories):
            ...
            if _lstat_kind(child) != "directory":
                raise V12DispatchError(f"output contains symlink or special directory: {child}")
            if frozen and stat.S_IMODE(child.stat().st_mode) != 0o555:
                raise V12DispatchError(f"output directory mode is not frozen 0555: {child}")
...
def _fresh(paths: ArtifactPaths) -> None:
    ...
    if existing:
        raise V12DispatchError(f"a one-use output or terminal artifact path already exists: {existing}")
...
def _acquire_lease(paths: ArtifactPaths) -> None:
    ...
    try:
        ...
        raise V12DispatchError("same-ID owner lease already exists") from error
    if _lstat_kind(paths.lease) != "directory" or list(paths.lease.iterdir()):
        raise V12DispatchError("same-ID owner lease is unsafe")


def _component_provenance() -> Mapping[str, Any]:
    """Bind every v12 component and fixed external to clean HEAD bytes."""
    try:
        if subprocess.run(["git", "-C", str(ROOT), "status", "--porcelain"], check=True, text=True, capture_output=True).stdout.strip():
            raise V12DispatchError("StateGuard3R tracked worktree is not clean")
        ...
        for relative in PINNED_COMPONENTS:
            ...
            if hashlib.sha256(head).digest() != hashlib.sha256(current).digest():
                raise V12DispatchError(f"pinned component differs from HEAD: {relative}")
        ...
        if subprocess.run(["git", "-C", str(RECAL3R_ROOT), "status", "--porcelain"], check=True, text=True, capture_output=True).stdout.strip():
            raise V12DispatchError("ReCal3R tracked worktree is not clean")
        ...
        if recal_commit != RECAL3R_COMMIT:
            raise V12DispatchError("ReCal3R commit differs from preregistered revision")
    except subprocess.CalledProcessError as error:
        raise V12DispatchError("cannot inspect pinned source provenance") from error
    external: dict[str, str] = {}
    for path in PINNED_EXTERNALS:
        if _lstat_kind(path) != "regular":
            raise V12DispatchError(f"pinned external input is missing or unsafe: {path}")
        external[str(path)] = _sha256_file(path)
    if external.get(str(CHECKPOINT)) != CHECKPOINT_SHA256:
        raise V12DispatchError("pinned checkpoint SHA-256 differs")
...
def _snapshot(*, require_minimum: bool = True) -> dict[str, Any]:
    ...
    if len(selected) != 1 or (require_minimum and selected[0]["memory_free_mib"] < MIN_FREE_MIB):
        raise V12DispatchError("GPU 2 UUID/free-memory precondition failed")
    if project:
        raise V12DispatchError("a StateGuard3R/ReCal3R project process is already on GPU 2")
...
def _validate_snapshot(snapshot: Mapping[str, Any], *, require_minimum: bool = True) -> None:
    ...
    if not isinstance(selected, Mapping) or selected.get("uuid") != GPU_UUID or type(selected.get("memory_free_mib")) is not int or (require_minimum and selected["memory_free_mib"] < MIN_FREE_MIB):
        raise V12DispatchError("stored GPU preflight snapshot is invalid")
    if not isinstance(snapshot.get("timestamp"), str) or not snapshot["timestamp"] or snapshot.get("project_gpu2_processes") != []:
        raise V12DispatchError("stored GPU snapshot lacks timestamp/project-process absence")
...
def _flag(argv: Sequence[str], name: str) -> str:
    ...
    if len(matches) != 1 or matches[0] + 1 >= len(argv):
        raise V12DispatchError(f"child must contain exactly one {name}")
...
def _gate_b_predecessor(run_id: str) -> None:
    ...
    if run_id != CANDIDATE_RUN_ID:
        raise V12DispatchError("v12 wrong/low runs remain forbidden until Gate C authorization")
    ...
    if _lstat_kind(control.validator) != "regular" or stat.S_IMODE(control.validator.stat().st_mode) != 0o444:
        raise V12DispatchError("dynamic candidate is blocked until frozen dynamic control validator PASS exists")
    ...
    if report.get("status") != "PASS" or report.get("dynamic_always_control", {}).get("status") != "PASS":
        raise V12DispatchError("dynamic candidate is blocked because dynamic control did not pass")
...
def _production_contract(argv: Sequence[str], paths: ArtifactPaths) -> None:
    ...
    if len(argv) != 2 + 2 * len(flags) or tuple(argv[2::2]) != flags:
        raise V12DispatchError("production child argv is not canonical v12 flag order")
    values = {argv[index]: argv[index + 1] for index in range(2, len(argv), 2)}
    spec = RUN_SPECS.get(paths.output.name)
    if spec is None:
        raise V12DispatchError("production v12 run ID is not preregistered")
    ...
    if not all(checks):
        raise V12DispatchError("production child differs from pinned v12 contract")
...
def _child(argv: Sequence[str], paths: ArtifactPaths, *, test: bool) -> list[str]:
    if not argv or any(type(item) is not str or not item or "\0" in item or "\n" in item or "\r" in item for item in argv):
        raise V12DispatchError("child argv is not a clean nonempty string array")
    if len(argv) < 2 or argv[1] not in {str(RUNNER_RELATIVE), str(ROOT / RUNNER_RELATIVE)}:
        raise V12DispatchError("child is not the exact pinned v12 runner")
    if _flag(argv, "--output-dir") != str(paths.output) or _flag(argv, "--state-policy") not in {"always-commit", CANDIDATE_POLICY}:
        raise V12DispatchError("child output or state policy is not pinned")
    ...
    if (not test and (argv[0] != str(RECAL3R_ROOT / ".venv" / "bin" / "python") or device != "cuda")) or (test and device not in {"cpu", "cuda"}):
        raise V12DispatchError("child interpreter/device does not satisfy v12 contract")
    child = list(argv)
    if not test:
...
def _interpreter_provenance(argv: Sequence[str], *, test: bool) -> Mapping[str, Any] | None:
    ...
    if _lstat_kind(executable) not in {"regular", "symlink"} or not executable.exists():
        raise V12DispatchError("pinned ReCal3R interpreter is unavailable")
...
def _driver(run_id: str, argv: Sequence[str], paths: ArtifactPaths, command_hash: str, quoted: str, token: str) -> str:
    if len(token) != 64 or any(ch not in "0123456789abcdef" for ch in token):
        raise V12DispatchError("driver go token is not 256-bit lowercase hex")
...
go_token={token_q}
record() {{ printf '%s\\n' "$1" | tee -a "$main_log"; }}
record "V12_DRIVER_START run_id=$run_id"
record "V12_DRIVER_COMMAND=$command_quoted"
record "V12_DRIVER_COMMAND_SHA256=$command_sha256"
record "V12_DRIVER_DISPATCHED run_id=$run_id kind=wrapper_only"
(
    gate_pid="$BASHPID"
...
if [[ ! -f "$start_path" ]]; then
    ...
    record "V12_DRIVER_PREEXEC_IDENTITY_FAILURE run_id=$run_id child_pid=$child_pid reason=start_gate_missing exit_code=$child_exit"; exit 70
fi
...
if [[ "$kind" != READY || "$reported_pid" != "$child_pid" || ! "$child_start_ticks" =~ ^[0-9]+$ ]]; then
    ...
    record "V12_DRIVER_PREEXEC_IDENTITY_FAILURE run_id=$run_id child_pid=$child_pid reason=start_time_unavailable exit_code=$child_exit"; exit 70
fi
driver_ticks="$(awk '{{print $22}}' "/proc/$child_pid/stat" 2>/dev/null)"
if [[ "$driver_ticks" != "$child_start_ticks" ]]; then
    kill "$child_pid" 2>/dev/null; wait "$child_pid"; child_exit=$?
    record "V12_DRIVER_PREEXEC_IDENTITY_FAILURE run_id=$run_id child_pid=$child_pid reason=parent_start_time_mismatch exit_code=$child_exit"; exit 70
fi
record "V12_DRIVER_CHILD_PID run_id=$run_id child_pid=$child_pid child_start_ticks=$child_start_ticks"
record "V12_DRIVER_PAYLOAD_RELEASE_ARMED run_id=$run_id child_pid=$child_pid child_start_ticks=$child_start_ticks"
if ! (set -o noclobber; printf '%s\\n' "$go_token" > "$go_path") 2>/dev/null; then
    kill "$child_pid" 2>/dev/null; wait "$child_pid"; child_exit=$?
    record "V12_DRIVER_PREEXEC_IDENTITY_FAILURE run_id=$run_id child_pid=$child_pid reason=go_gate_create_failed exit_code=$child_exit"; exit 70
fi
record "V12_DRIVER_PAYLOAD_RELEASED run_id=$run_id child_pid=$child_pid child_start_ticks=$child_start_ticks"
wait "$child_pid"; child_exit=$?
reaped_at="$(date --iso-8601=seconds)"
record "V12_DRIVER_EXIT run_id=$run_id child_pid=$child_pid child_start_ticks=$child_start_ticks exit_code=$child_exit reaped_at=$reaped_at"
if ! (set -o noclobber; : > "$result_path") 2>/dev/null; then record "V12_DRIVER_RESULT_CREATE_FAILED run_id=$run_id"; exit 70; fi
printf '{{"schema_version":"stateguard3r.v12-driver-result.v1","run_id":"%s","command_sha256":"%s","child_pid":%s,"child_start_ticks":"%s","exit_code":%s,"reaped_at":"%s"}}\\n' "$run_id" "$command_sha256" "$child_pid" "$child_start_ticks" "$child_exit" "$reaped_at" >> "$result_path"
record "V12_DRIVER_RESULT_WRITTEN run_id=$run_id child_pid=$child_pid child_start_ticks=$child_start_ticks exit_code=$child_exit"
exit 0
'''
...
def _pane() -> str:
    try:
        ...
        raise V12DispatchError("existing stateguard tmux session is unavailable") from error
    if not pane.startswith("%"):
        raise V12DispatchError("tmux did not return a fresh pane id")
    return pane
...
def _wait_for_text(path: Path, marker: str, deadline: float) -> None:
    while time.monotonic() < deadline:
        if _lexists(path) and marker in _read_regular_nul_free(path, label="tmux transcript").decode("utf-8", errors="strict"):
            return
        time.sleep(0.05)
    raise V12DispatchError(f"timed out waiting for ordered marker: {marker}")
...
def _wait_for_driver_exit(pane: str, deadline: float) -> None:
    while time.monotonic() < deadline:
        if not _pane_alive(pane):
            return
        time.sleep(0.05)
    raise V12DispatchError("driver pane did not exit after publishing its result")


def _close_pipe_and_drain(pane: str, transcript: Path, main: Path) -> Mapping[str, Any]:
    if _pane_alive(pane):
        raise V12DispatchError("driver pane is still live; terminal writers cannot be sealed")
    ...
    if first_t != second_t or first_m != second_m:
        raise V12DispatchError("terminal stream changed after driver pane closed")
...
def _partial_child_identity(path: Path, *, run_id: str) -> tuple[int, str] | None:
    ...
    except (UnicodeDecodeError, V12DispatchError):
        return None
    prefix = f"V12_DRIVER_CHILD_PID run_id={run_id} child_pid="
...
def _terminate_owned_child(identity: tuple[int, str], *, grace_seconds: float = 5.0) -> None:
    ...
    if _process_start_ticks(pid) != expected_ticks:
        raise V12DispatchError("timeout cannot prove the recorded child PID/start-time identity")
    ...
    while time.monotonic() < deadline:
        ...
        if observed != expected_ticks:
            raise V12DispatchError("child PID was reused while waiting for timeout termination")
        time.sleep(0.05)
    if _process_start_ticks(pid) == expected_ticks:
...
def _parse_result(path: Path, *, run_id: str, command_hash: str) -> Mapping[str, Any]:
    try:
        ...
        raise V12DispatchError("driver result is not valid UTF-8 JSON") from error
    expected = {"schema_version", "run_id", "command_sha256", "child_pid", "child_start_ticks", "exit_code", "reaped_at"}
    if not isinstance(result, dict) or set(result) != expected or result.get("schema_version") != "stateguard3r.v12-driver-result.v1" or result.get("run_id") != run_id or result.get("command_sha256") != command_hash or type(result.get("child_pid")) is not int or result["child_pid"] <= 0 or not isinstance(result.get("child_start_ticks"), str) or not result["child_start_ticks"].isdigit() or type(result.get("exit_code")) is not int or not isinstance(result.get("reaped_at"), str) or not result["reaped_at"]:
        raise V12DispatchError("driver result is malformed or unbound")
    return result
...
def _preexec_failure(path: Path, *, run_id: str) -> Mapping[str, Any] | None:
    try:
        ...
        return None
    marker = f"V12_DRIVER_PREEXEC_IDENTITY_FAILURE run_id={run_id} "
    found = [line for line in lines if line.startswith(marker)]
    if not found:
        return None
    if len(found) != 1 or any("V12_DRIVER_PAYLOAD_RELEASED" in line for line in lines):
        raise V12DispatchError("pre-exec identity evidence is ambiguous or released payload")
    ...
    if fields.get("run_id") != run_id or not fields.get("child_pid", "").isdigit() or fields.get("reason") not in {"start_gate_missing", "start_time_unavailable", "parent_start_time_mismatch", "go_gate_create_failed"} or not fields.get("exit_code", "").isdigit():
        raise V12DispatchError("pre-exec identity marker is malformed")
...
def _wait_for_result(path: Path, *, run_id: str, command_hash: str, deadline: float, main: Path) -> Mapping[str, Any]:
    ...
    while time.monotonic() < deadline:
        ...
        if failure is not None:
            raise V12PreexecNoGo(failure)
        if _lexists(path):
            try:
                return _parse_result(path, run_id=run_id, command_hash=command_hash)
            except V12DispatchError as error:
                last = error
        time.sleep(0.05)
    raise V12DispatchError(f"driver did not publish a complete exclusive result before timeout{': ' + str(last) if last else ''}")
...
def _ordered_stream(path: Path, *, run_id: str, quoted: str, command_hash: str, result: Mapping[str, Any] | None, transcript: bool) -> None:
    ...
    markers = [
        f"V12_DRIVER_START run_id={run_id}", f"V12_DRIVER_COMMAND={quoted}", f"V12_DRIVER_COMMAND_SHA256={command_hash}",
        f"V12_DRIVER_DISPATCHED run_id={run_id} kind=wrapper_only",
    ]
    if transcript:
        markers.insert(0, f"V12_PIPE_READY run_id={run_id}")
    if result is None:
        failure = _preexec_failure(path, run_id=run_id)
        if failure is None:
            raise V12DispatchError("pre-exec terminal stream has no identity failure")
        markers.append(f"V12_DRIVER_PREEXEC_IDENTITY_FAILURE run_id={run_id} child_pid={failure['child_pid']} reason={failure['reason']} exit_code={failure['exit_code']}")
    else:
        pid, ticks, exit_code = result["child_pid"], result["child_start_ticks"], result["exit_code"]
        markers.extend((
            f"V12_DRIVER_CHILD_PID run_id={run_id} child_pid={pid} child_start_ticks={ticks}",
            f"V12_DRIVER_PAYLOAD_RELEASE_ARMED run_id={run_id} child_pid={pid} child_start_ticks={ticks}",
            f"V12_DRIVER_PAYLOAD_RELEASED run_id={run_id} child_pid={pid} child_start_ticks={ticks}",
            f"V12_DRIVER_EXIT run_id={run_id} child_pid={pid} child_start_ticks={ticks} exit_code={exit_code}",
            f"V12_DRIVER_RESULT_WRITTEN run_id={run_id} child_pid={pid} child_start_ticks={ticks} exit_code={exit_code}",
        ))
    positions = [text.find(marker) for marker in markers]
    if any(position < 0 for position in positions) or positions != sorted(positions):
        raise V12DispatchError("terminal stream markers are missing or ill ordered")
...
def _json(path: Path, *, label: str) -> Mapping[str, Any]:
    try:
        ...
        raise V12DispatchError(f"{label} is not valid UTF-8 JSON") from error
    if not isinstance(value, Mapping):
        raise V12DispatchError(f"{label} must be a JSON object")
    return value
...
def _postflight(paths: ArtifactPaths, *, run_id: str, result: Mapping[str, Any] | None, source: Mapping[str, Any], pipe: Mapping[str, Any], error: str | None, token_sha256: str | None = None, preexec: Mapping[str, Any] | None = None) -> Mapping[str, Any]:
    payload: dict[str, Any] = {"schema_version": "stateguard3r.v12-postflight.v1", "run_id": run_id, "result": result, "source_provenance": source, "pipe_close_and_drain": pipe, "error": error}
    try:
...
def _control_check(paths: ArtifactPaths) -> Mapping[str, Any]:
    ...
    for name in PROTECTED_MODEL_OUTPUTS:
        old, new = _read_regular_nul_free(V1_DYNAMIC_CONTROL / name, label=f"v1 {name}"), _read_regular_nul_free(paths.output / name, label=f"v12 control {name}")
        equality[name] = old == new
        hashes[name] = {"v1_dynamic_sha256": hashlib.sha256(old).hexdigest(), "v12_control_sha256": hashlib.sha256(new).hexdigest()}
    if not all(equality.values()):
        raise V12DispatchError("v12 dynamic always-control protected outputs differ from v1")
    run, timeline = _json(paths.output / "run.json", label="v12 control run.json"), _json(paths.output / "state-timeline.json", label="v12 control timeline")
    ...
    if run.get("status") != "succeeded" or run.get("state_policy", {}).get("name") != "always-commit" or type(runtime) not in (int, float) or type(baseline) not in (int, float) or not math.isfinite(float(runtime)) or not math.isfinite(float(baseline)) or baseline <= 0:
        raise V12DispatchError("v12 control run metadata/runtime is invalid")
    ...
    if ratio > RUNTIME_RATIO_LIMIT or timeline.get("policy") != "always-commit" or timeline.get("pending_transaction_count") != 0 or not isinstance(transactions, list) or len(transactions) != 30:
        raise V12DispatchError("v12 control runtime or timeline does not pass")
    for frame_id, item in enumerate(transactions):
        if not isinstance(item, Mapping) or item.get("frame_id") != frame_id or item.get("action") != "commit" or item.get("reason") != "always_commit_control" or item.get("current_alarm") is not False or item.get("pending_transaction_count") != 0 or item.get("export_action") != "export_real_camera_pose":
            raise V12DispatchError("v12 control has a non-direct transaction")
...
def _validate_driver(paths: ArtifactPaths, *, run_id: str, argv: Sequence[str], command_hash: str, quoted: str) -> str:
    ...
    if len(tokens) != 1 or len(tokens[0]) != 64 or any(ch not in "0123456789abcdef" for ch in tokens[0]) or text != _driver(run_id, argv, paths, command_hash, quoted, tokens[0]):
        raise V12DispatchError("frozen driver differs from pinned token-gated template")
    return hashlib.sha256((tokens[0] + "\n").encode("ascii")).hexdigest()


def _validator_report(paths: ArtifactPaths, *, run_id: str) -> Mapping[str, Any]:
    report: dict[str, Any] = {"schema_version": "stateguard3r.v12-validator.v1", "run_id": run_id, "started_at": datetime.now().astimezone().isoformat(), "cpu_only": True}
    try:
        ...
        for path in primary:
            if _lstat_kind(path) != "regular" or stat.S_IMODE(path.stat().st_mode) != 0o444:
                raise V12DispatchError("sealed primary artifact mode/path invalid")
        ...
        if preflight.get("schema_version") != "stateguard3r.v12-preflight.v1" or preflight.get("run_id") != run_id or not isinstance(preflight.get("command"), list):
            raise V12DispatchError("sealed preflight schema is invalid")
        ...
        if quoted != shlex.join(argv) or command_hash != _command_hash(argv) or _child(argv, paths, test=preflight.get("test_child") is True) != argv:
            raise V12DispatchError("sealed child command does not recompute")
        snapshots = preflight.get("gpu_preflights")
        if not isinstance(snapshots, list) or len(snapshots) != 2:
            raise V12DispatchError("sealed preflight does not contain exactly two snapshots")
        ...
        if preflight.get("source_provenance") != source or postflight.get("source_provenance") != source:
            raise V12DispatchError("sealed source provenance does not recompute")
        ...
        if not _lexists(paths.result):
            ...
            if failure is None or _preexec_failure(paths.transcript, run_id=run_id) != failure or postflight.get("error") != "PREEXEC_IDENTITY_NO_GO" or postflight.get("preexec_no_go", {}).get("payload_not_executed") is not True or postflight["preexec_no_go"].get("go_token_sha256") != token_sha or postflight["preexec_no_go"].get("driver_result_exists") is not False or postflight["preexec_no_go"].get("output_kind") != "missing":
                raise V12DispatchError("sealed pre-exec no-go boundary does not recompute")
            ...
            if _lstat_kind(paths.result) != "regular" or stat.S_IMODE(paths.result.stat().st_mode) != 0o444:
                raise V12DispatchError("sealed result mode/path invalid")
            ...
            if postflight.get("result") != result or not postflight.get("pipe_close_and_drain", {}).get("drained") or "gpu_postflight_error" in postflight:
                raise V12DispatchError("sealed postflight is incomplete")
            ...
            if not isinstance(pid, Mapping) or pid.get("expected_start_ticks") != result["child_start_ticks"] or pid.get("pair_absent") is not True or pid.get("pid_reused") is not False:
                raise V12DispatchError("sealed postflight lacks PID/start-time absence proof")
            if result["exit_code"] == 0:
                inventory = _output_inventory(paths.output, frozen=True)
                if postflight.get("output_inventory") != inventory:
                    raise V12DispatchError("sealed output inventory does not recompute")
                if run_id == CONTROL_RUN_ID:
        ...
        raise V12DispatchError(f"independent v12 validator failed: {error}") from error
...
def validate_sealed_run(run_id: str) -> Mapping[str, Any]:
    ...
    if _lstat_kind(paths.lease) != "directory" or _lexists(paths.validator):
        raise V12DispatchError("validator requires owner lease and a fresh validator artifact")
...
def _run_independent_validator(run_id: str) -> Mapping[str, Any]:
    ...
    if result.returncode != 0:
        raise V12DispatchError(f"independent validator subprocess failed: {result.stderr.strip() or result.stdout.strip()}")
    try:
        return json.loads(result.stdout)
    except json.JSONDecodeError as error:
        raise V12DispatchError("independent validator did not emit JSON") from error
...
def dispatch(run_id: str, argv: Sequence[str], *, timeout_seconds: float, allow_noncuda_child_for_test: bool = False) -> Mapping[str, Any]:
    ...
    if timeout_seconds <= 0:
        raise V12DispatchError("dispatch timeout must be positive")
    paths = artifact_paths(run_id)
    _canonical_paths(paths)
    _acquire_lease(paths)
    existing = [name for name, value in asdict(paths).items() if name != "lease" and _lexists(value)]
    if existing:
        raise V12DispatchError(f"same-ID lease acquired but owner artifacts already exist: {existing}")
    ...
    try:
        ...
        for snapshot in snapshots:
            _validate_snapshot(snapshot)
        _write_json(paths.preflight, {"schema_version": "stateguard3r.v12-preflight.v1", "run_id": run_id, "command": child, "quoted_command": quoted, "command_sha256": command_hash, "test_child": allow_noncuda_child_for_test, "interpreter": _interpreter_provenance(child, test=allow_noncuda_child_for_test), "gpu_preflights": snapshots, "source_provenance": source})
        ...
        ready = f"V12_PIPE_READY run_id={run_id}"
        ...
        try:
        ...
        except V12PreexecNoGo as event:
            ...
            if _lexists(paths.result) or _lstat_kind(paths.output) != "missing":
                raise V12DispatchError("pre-exec identity failure unexpectedly has result or output")
            post_source = _component_provenance()
            if post_source != source:
                raise V12DispatchError("source/worktree drifted during pre-exec identity gate")
            ...
            if "gpu_postflight_error" in postflight:
                raise V12DispatchError("pre-exec no-go lacks GPU postflight snapshot")
            _validate_snapshot(postflight["gpu_postflight"], require_minimum=False)
            _write_json(paths.postflight, postflight)
            raise V12DispatchError("driver terminated before payload release because PID/start-time identity could not be established") from event
        except V12DispatchError as timeout_error:
            ...
            if identity is None:
                raise V12DispatchError("driver timed out without a PID/start-time marker; refusing to seal a possibly live child") from timeout_error
        ...
        if not pid["pair_absent"] or pid["pid_reused"]:
            raise V12DispatchError("recorded child PID/start-time pair is live or reused at postflight")
        post_source = _component_provenance()
        if post_source != source:
            raise V12DispatchError("source/worktree drifted during v12 child")
        postflight = dict(_postflight(paths, run_id=run_id, result=result, source=post_source, pipe=pipe, error=None))
        if "gpu_postflight_error" in postflight:
            raise V12DispatchError("postflight GPU snapshot could not be collected")
        _validate_snapshot(postflight["gpu_postflight"], require_minimum=False)
        ...
        if pane is None or (pipe is not None and pipe.get("drained") is True and pipe.get("pane_alive_at_close") is False):
            ...
            error = error or V12DispatchError("terminal writers were not closed/drained; refusing to freeze live evidence")
    try:
        primary = (paths.preflight, paths.main, paths.postflight, paths.transcript, paths.driver)
    ...
    if error is not None:
        raise V12DispatchError(f"v12 terminal evidence sealed as failure: {error}") from error
    assert result is not None
    if result["exit_code"] != 0:
        raise V12DispatchError(f"v12 child exited nonzero with immutable evidence: {result['exit_code']}")
...
def _read_command_json(path: Path) -> list[str]:
    if path.parent != ROOT / "tmp" or _lstat_kind(path) != "regular":
        raise V12DispatchError("command JSON must be a regular direct tmp child")
    try:
        value = json.loads(_read_regular_nul_free(path, label="command JSON"))
    except json.JSONDecodeError as error:
        raise V12DispatchError("command JSON is malformed") from error
    if not isinstance(value, list):
        raise V12DispatchError("command JSON must be an argv array")
    return value
    ...
    try:
        if args.validate_run_id is not None:
            ...
            if args.run_id is None:
                raise V12DispatchError("--run-id is required with --command-json")
        ...
        print(f"V12_DISPATCH_ERROR={error}", file=sys.stderr)
        return 70
