#!/usr/bin/env python3
"""Run the preregistered development-only v4 geometric-registration candidate."""
...
V4_COMPONENTS = (
    ROOT / "scripts" / "run_recal3r_geometric_registration_export_v4.py",
    ROOT / "src" / "stateguard3r" / "recal3r_geometric_registration_runner_v4.py",
    ROOT / "src" / "stateguard3r" / "geometric_registration_export_v4.py",
    ROOT / "src" / "stateguard3r" / "orb_pointmap_registration_v4.py",
    ROOT / "src" / "stateguard3r" / "geometric_registration_v4.py",
)
...
def _validate_args(args: argparse.Namespace, parser: argparse.ArgumentParser) -> None:
    if args.health_profile != "v3" or args.watchdog != 8:
        parser.error("v4 geometric registration requires fixed v3 health and watchdog=8")
    if args.recovery_quality_commitment is not None:
        parser.error("v4 geometric-registration development may not consume a recovery-quality commitment")
    if args.input_manifest is None:
        parser.error("v4 geometric registration requires a development input manifest")
    manifest = args.input_manifest.resolve(strict=True)
    if DEVELOPMENT_INPUT_ROOT.resolve() not in manifest.parents:
        parser.error("v4 geometric registration accepts only legacy development manifests")
    ...
    if not isinstance(payload, dict) or payload.get("schema_version") != "stateguard3r.corruption.v1" or payload.get("source_is_read_only") is not True:
        parser.error("v4 geometric registration requires a read-only legacy development manifest")
    if args.detector_config.resolve(strict=False) != FORMAL_V3_CONFIG.resolve(strict=False):
        parser.error("v4 geometric-registration detector config is fixed")
...
def _pose_sha256(value: Any) -> str:
    ...
    if not bool(tensor.isfinite().all()):
        raise RuntimeError("v4 pose audit tensor is nonfinite")
...
class _V4Observer:
    """Consume raw health, then externally export only real or registered poses."""

    def __init__(self, detector: Any, *, timestamps: Sequence[Any], torch: Any, pose_encoding_to_camera: Any, camera_to_pose_encoding: Any) -> None:
        from stateguard3r.geometric_registration_export_v4 import PinnedCameraPoseEncoder, RealSafeGeometricAnchor

        self._detector, self._timestamps, self._torch = detector, list(timestamps), torch
        self._decode = pose_encoding_to_camera
        self._previous_safe_camera: Any | None = None
        self._geometric = RealSafeGeometricAnchor(
            registered_pose_encoder=PinnedCameraPoseEncoder(
                torch=torch,
    ...
    def observe(self, frame_id: int, prediction: Mapping[str, Any], model: Any, frame_context: Mapping[str, Any]) -> _Observation:
        ...
        if len(records) != frame_id + 1:
            raise RuntimeError("v4 current health trace is not prefix aligned")
        overlap = frame_context.get("overlap")
        if frame_id == 0:
            if overlap is not None:
                raise RuntimeError("v4 overlap frame zero must be null")
        elif not isinstance(overlap, float) or not math.isfinite(overlap) or not 0.0 <= overlap <= 1.0:
            raise RuntimeError("v4 overlap must be finite in [0, 1]")
    ...
    def finalize(self, observation: _Observation, *, quarantined: bool, prediction: Mapping[str, Any]) -> tuple[Mapping[str, Any], Mapping[str, Any]]:
        ...
        return exported, {
            ...
            "anchor_frame_ids": None if action.anchor_frame_id is None else [action.anchor_frame_id],
            "registration": action.registration_evidence,
            "anchor_real_pose_sha256": action.anchor_real_pose_sha256,
...
def _before_frame(overlaps: Any, frame_id: int, view: Mapping[str, Any]) -> Mapping[str, Any]:
    context = dict(overlaps.before_frame(frame_id, view))
    image = view.get("img")
    ...
    context["v4_rgb"] = image.detach().clone()
    return context
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if requested.exists() or requested.parent != ROOT / "outputs":
        raise RuntimeError("v4 geometric-registration output must be a new direct outputs child")
    ...
    from stateguard3r.recal3r_geometric_registration_runner_v4 import run_geometric_registration_recurrent_lighter
    ...
    try:
        ...
        model, audit = smoke._load_model_with_state_dict_audit(dust3r_model.ARCroco3DStereo, args.checkpoint, torch)
        smoke._write_json_atomic(staging / "checkpoint-load-audit.json", audit)
        if audit["strict"] is not False or audit["missing_keys"] or audit["unexpected_keys"]:
        ...
        if args.state_policy != "always-commit":
            observer = _V4Observer(
                IncrementalOnlinePrefixDetector(FrozenDetectorV3Config.from_mapping(config_payload), captures=captures, capture_provenance=provenance),
                timestamps=smoke._input_timestamps(args),
                torch=torch,
                pose_encoding_to_camera=camera.pose_encoding_to_camera,
                camera_to_pose_encoding=camera.camera_to_pose_encoding,
            )
        ...
        with torch.no_grad():
            result = run_geometric_registration_recurrent_lighter(
                ...
                before_frame=lambda frame_id, view: _before_frame(overlaps, frame_id, view),
                observer=observer, watchdog_limit=args.watchdog, synchronize=synchronize,
            )
        if device.type == "cuda":
            torch.cuda.synchronize(device)
        wall_elapsed = time.perf_counter() - started
        if updates != len(views) - 1 or len(result.predictions) != len(views) or len(overlaps.values) != len(views):
            raise RuntimeError("v4 recurrent accounting differs")
        ...
        if observer is None:
            ...
            if len(health) != len(views) or len(ledger) != len(views):
                raise RuntimeError("v4 observed health ledger is incomplete")
            trajectory_payload = {"schema_version": "stateguard3r.trajectory.geometric-registration-v4", "pose_encoding": "absT_quaR", "matrix_convention": "camera_to_first_input_frame_reference", "camera_pose_semantics": "clear frames export their real camera pose; alarm frames export only anchor-ORB-3D3D registration; health remains raw", "registration_evidence_location": "state-timeline.json transactions", "frames": trajectory}
        smoke._verify_preflight_inputs(args)
        ...
        state_timeline = {"schema_version": SCHEMA_VERSION, "policy": args.state_policy, "watchdog": args.watchdog, "causality": "detector consumes current/past raw candidate health, adjacent model-ready RGB overlap, and current/past capture timestamps; registration consumes only a past external real-safe RGB/reference pointmap and current RGB/self pointmap; registered exports never feed model, detector, or future anchor", "pending_transaction_count": 0, "source_provenance": result.source_provenance, "transactions": result.timeline}
        smoke._write_json_atomic(staging / "state-timeline.json", state_timeline)
        finished = datetime.now().astimezone().isoformat()
        metadata = {"schema_version": SCHEMA_VERSION, "status": "succeeded", "argv": sys.argv if argv is None else [str(item) for item in argv], "baseline_root": str(args.baseline_root), "baseline_commit": args.baseline_commit, "baseline_tracked_worktree_clean": True, "module_paths": module_paths, "runner": runner_provenance, "v4_components": component_provenance, "checkpoint": str(args.checkpoint), "checkpoint_sha256": args.checkpoint_sha256, "input_manifest": smoke._input_manifest_metadata(args), "safe_input_frames": safe_frames, "frame_count": len(views), "size": args.size, "seed": args.seed, "model_update_type": "recal3r", "loaded_model_interface": interface, "checkpoint_state_dict": audit, "beta_base": args.beta_base, "recal3r_runtime_config": config, "model_eval": not model.training, "calibrated_update_calls": updates, "health_profile": "v4-gpu-current-raw-candidate", "online_visual_correspondence": overlaps.metadata(), "online_detector": {"config": config_artifact, "allowed_inputs": ["candidate_model_health", "previous_current_model_ready_rgb_overlap", "raw_rgb_capture_timestamp"], "forbidden": ["GT", "depth", "label", "event", "source_index", "future_frame", "baseline_alarm_artifact"]}, "state_policy": {"name": args.state_policy, "watchdog": args.watchdog, "timeline_path": str(requested / "state-timeline.json"), "timeline_sha256": smoke._sha256(staging / "state-timeline.json")}, "runtime_seconds": result.recurrent_policy_runtime_seconds, "wall_runtime_seconds": wall_elapsed, "runtime_scope": "per_frame_cuda_synchronized_v4_recurrent_policy_excluding_common_causal_rgb_overlap", "started_at": started_at, "finished_at": finished, "pid": os.getpid(), "output_dir": str(requested), "peak_memory_allocated_mib": float(torch.cuda.max_memory_allocated(device) / 1024**2) if device.type == "cuda" else 0.0, "device": str(device), "cuda_visible_devices": os.environ.get("CUDA_VISIBLE_DEVICES"), "gpu_name": torch.cuda.get_device_name(device) if device.type == "cuda" else None, "torch_version": torch.__version__, "cuda_runtime": torch.version.cuda}
