#!/usr/bin/env python3
"""Run the preregistered v8 early-decoder pose export candidate."""
...
V8_COMPONENTS = (
    ...
    ROOT / "src" / "stateguard3r" / "recal3r_early_decoder_pose_runner_v8.py",
    ROOT / "src" / "stateguard3r" / "early_decoder_pose_export_v8.py",
    ROOT / "src" / "stateguard3r" / "early_decoder_pose_v8.py",
)
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--state-policy",
        choices=("always-commit", "detector-v3-incremental-early-decoder-pose-export"),
        required=True,
    )
...
def _validate_args(args: argparse.Namespace, parser: argparse.ArgumentParser) -> None:
    if args.health_profile != "v3" or args.watchdog != 8:
        parser.error("v8 early decoder pose requires fixed v3 health and watchdog=8")
    if args.recovery_quality_commitment is not None:
        parser.error("v8 development may not consume a recovery-quality commitment")
    ...
    if not isinstance(payload, dict) or payload.get("schema_version") != "stateguard3r.corruption.v1" or payload.get("source_is_read_only") is not True:
        parser.error("v8 early decoder pose requires a read-only development manifest")
    if args.detector_config.resolve(strict=False) != FORMAL_V3_CONFIG.resolve(strict=False):
        parser.error("v8 early decoder pose detector config is fixed")
...
def _self_provenance() -> dict[str, Any]:
    try:
        ...
        raise RuntimeError(f"cannot inspect v8 runner provenance: {error}") from error
    if changes:
        raise RuntimeError("StateGuard3R has tracked changes; v8 runner refuses execution")
    for component in V8_COMPONENTS:
        try:
            ...
            raise RuntimeError(f"v8 component is not tracked at HEAD: {component}") from error
        if committed_blob != working_blob:
            raise RuntimeError(f"v8 component differs from its HEAD blob: {component}")
    return {
...
def _pinned_pose_head_interface(model: Any, dpt_head_module: Any) -> tuple[Any, Any]:
    ...
    if expected is None or type(downstream) is not expected:
        raise RuntimeError("v8 model downstream head is not pinned DPTPts3dPose")
    ...
    if not callable(pose_head) or not isinstance(pose_mode, tuple) or len(pose_mode) != 3:
        raise RuntimeError("v8 pinned DPT pose-head interface is unavailable")
...
class _Observation:
    ...
    decision: Any
    raw_pose: Any
    early_decoder_pose_token: Any
    token_source: str


class _V8Observer:
    """Keep health raw; decode a captured early decoder token after rollback."""
    ...
    def observe(self, frame_id: int, prediction: Mapping[str, Any], model: Any, frame_context: Mapping[str, Any], early_decoder_pose_token: Any) -> _Observation:
        ...
        if len(records) != frame_id + 1:
            raise RuntimeError("v8 current health trace is not prefix aligned")
        overlap = frame_context.get("overlap")
        if frame_id == 0:
            if overlap is not None:
                raise RuntimeError("v8 overlap frame zero must be null")
        elif not isinstance(overlap, float) or not math.isfinite(overlap) or not 0.0 <= overlap <= 1.0:
            raise RuntimeError("v8 overlap must be finite in [0, 1]")
        ...
        if early_decoder_pose_token is None:
            raise RuntimeError("v8 candidate lacks the required early decoder token")
        return _Observation(frame_id, record, self._detector.observe(record), pose, early_decoder_pose_token, EARLY_DECODER_TOKEN_SOURCE)
    ...
    def finalize(self, observation: _Observation, *, quarantined: bool, prediction: Mapping[str, Any]) -> tuple[Mapping[str, Any], Mapping[str, Any]]:
        ...
        if quarantined:
            ...
            exported, action = self._exporter.export_alarm(observation.frame_id, prediction, observation.early_decoder_pose_token, token_source=observation.token_source)
        else:
        ...
        token_evidence = dict(action.evidence) if action.evidence is not None else None
        if token_evidence is not None:
            token_evidence["source_hashes"] = self._source_hashes
        return exported, {
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    import numpy as np
    import torch
    from stateguard3r.early_decoder_pose_export_v8 import EarlyDecoderPoseExport
    from stateguard3r.early_decoder_pose_v8 import audit_pinned_early_decoder_pose_sources
    ...
    try:
        ...
        if args.state_policy != "always-commit":
            observer = _V8Observer(
                ...
                torch=torch,
                pose_encoding_to_camera=camera.pose_encoding_to_camera,
                source_hashes={key: early_decoder_pose_source_audit[key] for key in ("model_sha256", "dpt_head_sha256", "postprocess_sha256", "runner_sha256")},
            )
        ...
        with torch.no_grad():
            result = run_early_decoder_pose_recurrent_lighter(
        ...
        if updates != len(views) - 1 or len(result.predictions) != len(views) or len(overlaps.values) != len(views):
            raise RuntimeError("v8 recurrent accounting differs")
        ...
        if observer is None:
            ...
            if len(health) != len(views) or len(ledger) != len(views):
                raise RuntimeError("v8 observed health ledger is incomplete")
            trajectory_payload = {"schema_version": "stateguard3r.trajectory.early-decoder-pose-v8", "pose_encoding": "absT_quaR", "matrix_convention": "camera_to_first_input_frame_reference", "camera_pose_semantics": "clear frames export real raw pose; alarm frames export only captured early-decoder layer-0 token decode; detector health remains raw", "early_decoder_token_evidence_location": "state-timeline.json transactions", "frames": trajectory}
        smoke._verify_preflight_inputs(args)
        ...
        metadata = {
            ...
            "health_profile": "v8-gpu-current-raw-candidate",
            ...
            "runtime_scope": "per_frame_cuda_synchronized_v8_recurrent_policy_including_candidate_early_decoder_token_export_excluding_common_causal_rgb_overlap",
