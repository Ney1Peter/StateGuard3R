#!/usr/bin/env python3
"""Evaluate the sealed 12-response Stage 0.8 feasibility calibration once."""
...
PROTOCOL_SCHEMA_VERSION = "stateguard3r.state-triage-stage0p8-calibration-gate-b.v1"
RESULT_SCHEMA_VERSION = "stateguard3r.state-triage-stage0p8-calibration-evaluation.v1"
...
class Stage0p8CalibrationEvaluationError(ValueError):
    pass
...
def _json(path: Path) -> dict[str, Any]:
    def reject_constant(value: str) -> None:
        raise Stage0p8CalibrationEvaluationError(f"non-finite JSON constant {value!r}")
    def duplicate(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
        result: dict[str, Any] = {}
        for key, value in pairs:
            if key in result:
                raise Stage0p8CalibrationEvaluationError(f"duplicate JSON key {key!r}")
            result[key] = value
        return result
    try:
        value = json.loads(path.read_text(encoding="utf-8"), parse_constant=reject_constant, object_pairs_hook=duplicate)
    except (OSError, UnicodeDecodeError, json.JSONDecodeError, Stage0p8CalibrationEvaluationError) as error:
        raise Stage0p8CalibrationEvaluationError(f"cannot load strict JSON {path}: {error}") from error
    if not isinstance(value, dict):
        raise Stage0p8CalibrationEvaluationError(f"{path} must contain a JSON object")
    return value


def _finite(value: Any, name: str) -> float:
    if isinstance(value, bool) or not isinstance(value, (int, float)) or not math.isfinite(float(value)):
        raise Stage0p8CalibrationEvaluationError(f"{name} must be finite")
    return float(value)
...
def _read_only(path: Path, *, directory: bool = False) -> None:
    ...
    if stat.S_ISLNK(metadata.st_mode) or (not stat.S_ISDIR(metadata.st_mode) if directory else not stat.S_ISREG(metadata.st_mode)) or stat.S_IMODE(metadata.st_mode) & 0o222:
        raise Stage0p8CalibrationEvaluationError(f"{path} must be a read-only {'directory' if directory else 'file'}")


def _direct(relative: str, parent: Path) -> Path:
    path = (ROOT / relative).resolve(strict=True)
    if path.parent != parent:
        raise Stage0p8CalibrationEvaluationError(f"{relative} lies outside approved direct parent")
    return path
...
def _protocol(path: Path) -> dict[str, Any]:
    ...
    if set(value) != required or value.get("schema_version") != PROTOCOL_SCHEMA_VERSION or value.get("stage") != "GATE_B_STAGE0P8_CALIBRATION_FROZEN_PRE_EVALUATION":
        raise Stage0p8CalibrationEvaluationError("protocol schema differs")
    artifact = value["input_inventory"]
    if not isinstance(artifact, Mapping) or set(artifact) != {"path", "sha256"}:
        raise Stage0p8CalibrationEvaluationError("input artifact schema differs")
    ...
    if _sha(inputs / "protocol.json") != artifact["sha256"]:
        raise Stage0p8CalibrationEvaluationError("input inventory hash differs")
    ...
    if not isinstance(sources, Mapping) or set(sources) != {"runner_path", "runner_sha256", "observer_path", "observer_sha256"}:
        raise Stage0p8CalibrationEvaluationError("source schema differs")
    ...
    if _sha(runner) != sources["runner_sha256"] or _sha(observer) != sources["observer_sha256"]:
        raise Stage0p8CalibrationEvaluationError("runner or observer changed after protocol freeze")
    if value["decision"] != DECISION_CONTRACT or value["interaction"] != INTERACTION_CONTRACT or value["frozen_evidence_config"] != EvidenceConfig().to_dict():
        raise Stage0p8CalibrationEvaluationError("decision, interaction, or evidence contract differs")
    items = value["calibration_inventory"]
    expected_item = {"capsule_path", "capsule_sha256", "capsule_id", "output"}
    if not isinstance(items, list) or len(items) != 12 or any(not isinstance(item, Mapping) or set(item) != expected_item for item in items) or len({item["capsule_id"] for item in items}) != 12 or len({item["output"] for item in items}) != 12:
        raise Stage0p8CalibrationEvaluationError("12-response inventory schema/uniqueness differs")
    gates = value["gates"]
    ...
    if not isinstance(gates, Mapping) or set(gates) != expected_gates or gates["valid_event_count"] != 12 or gates["activated_event_min"] != 10:
        raise Stage0p8CalibrationEvaluationError("calibration gate schema differs")
...
def _event(item: Mapping[str, Any], protocol: Mapping[str, Any]) -> dict[str, Any]:
    capsule_path = _direct(str(item["capsule_path"]), ROOT / "outputs" / "state-triage-v2-stage0p8-inputs-0001" / "calibration")
    _read_only(capsule_path)
    if _sha(capsule_path) != item["capsule_sha256"]:
        raise Stage0p8CalibrationEvaluationError("calibration capsule hash differs")
    ...
    if capsule.capsule_id != item["capsule_id"] or capsule.event.cause != "transient_local_content":
        raise Stage0p8CalibrationEvaluationError("calibration capsule identity differs")
    ...
    if run.get("status") != "succeeded" or run.get("observer") != "on" or not required_run.issubset(run) or not isinstance(run["input_capsule"], Mapping) or run["input_capsule"].get("sha256") != item["capsule_sha256"] or run["frame_count"] != len(capsule.frames):
        raise Stage0p8CalibrationEvaluationError("run provenance differs")
    for field in ("observer_wall_seconds", "runtime_seconds", "peak_memory_allocated_mib"):
        if _finite(run[field], field) < 0.0:
            raise Stage0p8CalibrationEvaluationError(f"{field} cannot be negative")
    if evidence.get("config") != protocol["frozen_evidence_config"] or evidence.get("causality") != "each row accepts current frame plus strict observer prefix only":
        raise Stage0p8CalibrationEvaluationError("evidence causality/config differs")
    ...
    if not isinstance(rows, list) or len(rows) != len(capsule.frames) or [row.get("frame_id") for row in rows if isinstance(row, Mapping)] != list(range(len(rows))):
        raise Stage0p8CalibrationEvaluationError("evidence rows differ")
    first = rows[capsule.event.start_frame]
    if not isinstance(first, Mapping):
        raise Stage0p8CalibrationEvaluationError("first event evidence row is malformed")
    features = {name: _finite(first.get(name), f"first event {name}") for name in ("sharpness", "clipped_fraction", "coverage_ratio")}
    return {"event_id": capsule.event.event_id, "capsule_id": capsule.capsule_id, "source_sequence": capsule.source_sequence, "truth": capsule.event.cause, "event_window": {"start_frame": capsule.event.start_frame, "end_frame": capsule.event.end_frame}, "first_event_evidence": features, "activated": features["sharpness"] > float(protocol["gates"]["sharpness_min_exclusive"]) and features["clipped_fraction"] >= float(protocol["gates"]["clipped_fraction_min"]) and features["coverage_ratio"] < float(protocol["gates"]["coverage_max_exclusive"]), "run": {"output": output.name, "runner_commit": run["runner"].get("commit"), "observer_wall_seconds": run["observer_wall_seconds"], "runtime_seconds": run["runtime_seconds"], "peak_memory_allocated_mib": run["peak_memory_allocated_mib"]}, "decisions": {method: decide_event(rows, method=method, start_frame=capsule.event.start_frame, end_frame=capsule.event.end_frame) for method in METHODS}}
...
def evaluate(*, protocol_path: Path, output_dir: Path) -> Path:
    ...
    if protocol_path.parent != ROOT / "docs" / "protocols" or output_dir.parent != ROOT / "outputs" or output_dir.exists():
        raise Stage0p8CalibrationEvaluationError("protocol/output location is invalid")
    ...
    result = {"schema_version": RESULT_SCHEMA_VERSION, "status": "P0.8_CALIBRATION_ACCEPTED" if all(gates.values()) else "P0.8_CALIBRATION_REJECTED", "evaluated_at": datetime.now().astimezone().isoformat(), "protocol": {"path": str(protocol_path.relative_to(ROOT)), "sha256": _sha(protocol_path)}, "decision_contract": DECISION_CONTRACT, "interaction_contract": INTERACTION_CONTRACT, "events": events, "activation_count": sum(bool(event["activated"]) for event in events), "gates": gates, "scope": "recipe-feasibility calibration only; no final benchmark score or method claim"}
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--protocol", type=Path, default=ROOT / "docs" / "protocols" / "state-triage-v2-stage0p8-calibration-gate-b.json")
    parser.add_argument("--output-dir", type=Path, default=ROOT / "outputs" / "state-triage-v2-stage0p8-calibration-evaluation-0001")
    args = parser.parse_args(argv)
    try:
        print(evaluate(protocol_path=args.protocol, output_dir=args.output_dir))
    except (Stage0p8CalibrationEvaluationError, OSError, ValueError) as error:
