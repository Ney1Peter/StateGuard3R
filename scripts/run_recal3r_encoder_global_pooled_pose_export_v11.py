#!/usr/bin/env python3
"""Run the preregistered v11 encoder-global alarm pose-export route.

The candidate captures a current-frame encoder global only before retrieval
and recurrence.  Detector-v3 continues to consume the unmodified raw model
prediction; an alarm replaces only the exported camera pose after rollback.
"""
...
smoke = v2.smoke
SCHEMA_VERSION = "stateguard3r.recal3r-encoder-global-pooled-pose-export-v11.v1"
...
V11_COMPONENTS = (
    ROOT / "scripts" / "dispatch_recal3r_encoder_global_pooled_pose_v11.py",
    ROOT / "scripts" / "run_recal3r_encoder_global_pooled_pose_export_v11.py",
    ROOT / "src" / "stateguard3r" / "recal3r_encoder_global_pooled_pose_runner_v11.py",
    ROOT / "src" / "stateguard3r" / "encoder_global_pooled_pose_export_v11.py",
    ROOT / "src" / "stateguard3r" / "encoder_global_pooled_pose_v11.py",
)
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--state-policy",
        choices=("always-commit", "detector-v3-incremental-encoder-global-pooled-pose-export"),
        required=True,
    )
...
def _validate_args(args: argparse.Namespace, parser: argparse.ArgumentParser) -> None:
    if args.health_profile != "v3" or args.watchdog != 8:
        parser.error("v11 encoder-global pose requires fixed v3 health and watchdog=8")
    if args.recovery_quality_commitment is not None:
        parser.error("v11 development may not consume a recovery-quality commitment")
    if args.input_manifest is None:
        parser.error("v11 encoder-global pose requires a development input manifest")
    ...
    if manifest not in {path.resolve(strict=True) for path in DEVELOPMENT_MANIFESTS}:
        parser.error("v11 encoder-global pose accepts only disclosed development manifests")
    ...
    if not isinstance(payload, dict) or payload.get("schema_version") != "stateguard3r.corruption.v1" or payload.get("source_is_read_only") is not True:
        parser.error("v11 encoder-global pose requires a read-only development manifest")
    if args.detector_config.resolve(strict=False) != FORMAL_V3_CONFIG.resolve(strict=False):
        parser.error("v11 encoder-global pose detector config is fixed")
...
def _self_provenance() -> dict[str, Any]:
    try:
        ...
        raise RuntimeError(f"cannot inspect v11 runner provenance: {error}") from error
    if changes:
        raise RuntimeError("StateGuard3R has tracked changes; v11 runner refuses execution")
    for component in V11_COMPONENTS:
        try:
            ...
            raise RuntimeError(f"v11 component is not tracked at HEAD: {component}") from error
    return {
...
def _component_provenance() -> list[dict[str, str]]:
    return [{"path": str(path.resolve(strict=True)), "sha256": smoke._sha256(path.resolve(strict=True))} for path in V11_COMPONENTS]


def _validate_new_output_dir(requested: Path) -> None:
    if requested.exists() or requested.parent != ROOT / "outputs":
        raise RuntimeError("v11 encoder-global output must be a new direct outputs child")
...
def _pose_gpu_digest(value: Any, *, torch: Any) -> str:
    ...
    if not bool(tensor.isfinite().all()):
        raise RuntimeError("v11 pose audit tensor is nonfinite")
...
class _V11Observer:
    """Feed raw health to Detector-v3 and export only the captured v11 token."""
    ...
    def observe(self, frame_id: int, prediction: Mapping[str, Any], model: Any, frame_context: Mapping[str, Any], capture: Any) -> _Observation:
        ...
        if not isinstance(frame_context, Mapping) or not isinstance(frame_context.get("v11_loaded_image_shape"), list) or len(frame_context["v11_loaded_image_shape"]) != 2:
            raise RuntimeError("v11 candidate lacks loaded-image shape evidence")
        loaded_shape = frame_context["v11_loaded_image_shape"]
        if any(type(value) is not int or value <= 0 for value in loaded_shape):
            raise RuntimeError("v11 candidate loaded-image shape is invalid")
        ...
        if len(records) != frame_id + 1:
            raise RuntimeError("v11 current health trace is not prefix aligned")
        ...
        if (frame_id == 0 and overlap is not None) or (frame_id and (not isinstance(overlap, float) or not math.isfinite(overlap) or not 0.0 <= overlap <= 1.0)):
            raise RuntimeError("v11 overlap is not a valid causal current/previous value")
        ...
        return _Observation(frame_id, record, self._detector.observe(record), raw_pose, capture, list(loaded_shape))
    ...
    def finalize(self, observation: _Observation, *, quarantined: bool, prediction: Mapping[str, Any]) -> tuple[Mapping[str, Any], Mapping[str, Any]]:
        ...
        return exported, {"export_action": action.action, "encoder_global_pose_token": evidence, "raw_candidate_pose_digest": raw_digest, "exported_camera_pose_digest": _pose_gpu_digest(exported["camera_pose"], torch=self._torch)}
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    import numpy as np
    import torch
    from stateguard3r.encoder_global_pooled_pose_export_v11 import EncoderGlobalPooledPoseExport
    from stateguard3r.encoder_global_pooled_pose_v11 import audit_pinned_encoder_global_pose_sources
    ...
    from stateguard3r.recal3r_encoder_global_pooled_pose_runner_v11 import audit_v11_runner_contract, run_encoder_global_pooled_pose_recurrent_lighter
    ...
    source_audit = audit_pinned_encoder_global_pose_sources(dust3r / "model.py", dust3r / "heads" / "dpt_head.py", dust3r / "heads" / "postprocess.py", dust3r / "patch_embed.py", dust3r / "utils" / "image.py", ROOT / "src" / "stateguard3r" / "recal3r_encoder_global_pooled_pose_runner_v11.py")
    runner_audit = audit_v11_runner_contract(ROOT / "src" / "stateguard3r" / "recal3r_encoder_global_pooled_pose_runner_v11.py")
    ...
    try:
        ...
        if args.state_policy != "always-commit":
            observer = _V11Observer(IncrementalOnlinePrefixDetector(FrozenDetectorV3Config.from_mapping(config_payload), captures=captures, capture_provenance=capture_provenance), EncoderGlobalPooledPoseExport(torch=torch, pose_head=pose_head, postprocess_pose=pose_postprocess.postprocess_pose, pose_mode=pose_mode, pose_encoding_to_camera=camera.pose_encoding_to_camera), timestamps=smoke._input_timestamps(args), torch=torch, pose_encoding_to_camera=camera.pose_encoding_to_camera, source_hashes={key: source_audit[key] for key in ("model_sha256", "dpt_head_sha256", "postprocess_sha256", "patch_embed_sha256", "image_loader_sha256", "runner_sha256")})
        ...
        if observer is None:
            ...
            if len(health) != len(views) or len(ledger) != len(views):
                raise RuntimeError("v11 observed health ledger is incomplete")
            trajectory_payload = {"schema_version": "stateguard3r.trajectory.encoder-global-pooled-pose-v11", "pose_encoding": "absT_quaR", "matrix_convention": "camera_to_first_input_frame_reference", "camera_pose_semantics": "clear frames export real raw pose; alarm frames export only current encoder-global token after rollback; detector health remains raw", "encoder_global_token_evidence_location": "state-timeline.json transactions", "frames": trajectory}
        smoke._verify_preflight_inputs(args)
