#!/usr/bin/env python3
"""Run the preregistered v5 current self/cross-pointmap export candidate."""
...
smoke = v2.smoke
SCHEMA_VERSION = "stateguard3r.recal3r-current-pointmap-consensus-export-v5.v1"
...
V5_COMPONENTS = (
    ROOT / "scripts" / "run_recal3r_current_pointmap_consensus_export_v5.py",
    ROOT / "src" / "stateguard3r" / "recal3r_current_pointmap_runner_v5.py",
    ROOT / "src" / "stateguard3r" / "current_pointmap_consensus_export_v5.py",
    ROOT / "src" / "stateguard3r" / "current_pointmap_consensus_v5.py",
)
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--state-policy",
        choices=("always-commit", "detector-v3-incremental-current-self-cross-consensus-export"),
        required=True,
    )
...
def _validate_args(args: argparse.Namespace, parser: argparse.ArgumentParser) -> None:
    if args.health_profile != "v3" or args.watchdog != 8:
        parser.error("v5 current-pointmap consensus requires fixed v3 health and watchdog=8")
    if args.recovery_quality_commitment is not None:
        parser.error("v5 development may not consume a recovery-quality commitment")
    if args.input_manifest is None:
        parser.error("v5 current-pointmap consensus requires a development input manifest")
    manifest = args.input_manifest.resolve(strict=True)
    if DEVELOPMENT_INPUT_ROOT.resolve() not in manifest.parents:
        parser.error("v5 current-pointmap consensus accepts only disclosed development manifests")
    ...
    if not isinstance(payload, dict) or payload.get("schema_version") != "stateguard3r.corruption.v1" or payload.get("source_is_read_only") is not True:
        parser.error("v5 current-pointmap consensus requires a read-only development manifest")
    if args.detector_config.resolve(strict=False) != FORMAL_V3_CONFIG.resolve(strict=False):
        parser.error("v5 current-pointmap consensus detector config is fixed")
...
def _self_provenance() -> dict[str, Any]:
    try:
        ...
        raise RuntimeError(f"cannot inspect v5 runner provenance: {error}") from error
    if changes:
        raise RuntimeError("StateGuard3R has tracked changes; v5 runner refuses execution")
    return {
...
def _pose_sha256(value: Any) -> str:
    ...
    if not bool(tensor.isfinite().all()):
        raise RuntimeError("v5 pose audit tensor is nonfinite")
...
class _V5Observer:
    """Keep health on raw predictions and replace an alarm export only afterward."""
    ...
    def observe(self, frame_id: int, prediction: Mapping[str, Any], model: Any, frame_context: Mapping[str, Any]) -> _Observation:
        ...
        if len(records) != frame_id + 1:
            raise RuntimeError("v5 current health trace is not prefix aligned")
        overlap = frame_context.get("overlap")
        if frame_id == 0:
            if overlap is not None:
                raise RuntimeError("v5 overlap frame zero must be null")
        elif not isinstance(overlap, float) or not math.isfinite(overlap) or not 0.0 <= overlap <= 1.0:
            raise RuntimeError("v5 overlap must be finite in [0, 1]")
    ...
    def finalize(self, observation: _Observation, *, quarantined: bool, prediction: Mapping[str, Any]) -> tuple[Mapping[str, Any], Mapping[str, Any]]:
        ...
        if quarantined:
            ...
            exported, action = self._exporter.export_alarm(observation.frame_id, prediction)
        else:
        ...
        return exported, {
            ...
            "current_pointmap_consensus": action.evidence,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if requested.exists() or requested.parent != ROOT / "outputs":
        raise RuntimeError("v5 current-pointmap output must be a new direct outputs child")
    ...
    from stateguard3r.recal3r_current_pointmap_runner_v5 import run_current_pointmap_recurrent_lighter
    ...
    try:
        ...
        with torch.no_grad():
            result = run_current_pointmap_recurrent_lighter(
        ...
        if updates != len(views) - 1 or len(result.predictions) != len(views) or len(overlaps.values) != len(views):
            raise RuntimeError("v5 recurrent accounting differs")
        ...
        if observer is None:
            ...
            if len(health) != len(views) or len(ledger) != len(views):
                raise RuntimeError("v5 observed health ledger is incomplete")
            trajectory_payload = {"schema_version": "stateguard3r.trajectory.current-pointmap-consensus-v5", "pose_encoding": "absT_quaR", "matrix_convention": "camera_to_first_input_frame_reference", "camera_pose_semantics": "clear frames export real raw pose; alarm frames export only current self/cross pointmap consensus; detector health remains raw", "consensus_evidence_location": "state-timeline.json transactions", "frames": trajectory}
        smoke._verify_preflight_inputs(args)
        ...
        state_timeline = {
            ...
            "causality": "detector consumes current/past raw candidate health, adjacent model-ready RGB overlap, and current/past capture timestamps; consensus export consumes only current self/cross pointmaps and confidences; raw camera_pose supplies encoder device/dtype template only; no export feeds model, detector, state, or later frame",
            "cross_head_source_audit": cross_head_audit,
        ...
        metadata = {
            ...
            "v5_components": component_provenance,
            "cross_head_source_audit": cross_head_audit,
            ...
            "health_profile": "v5-gpu-current-raw-candidate",
            ...
            "runtime_scope": "per_frame_cuda_synchronized_v5_recurrent_policy_including_current_gpu_consensus_excluding_common_causal_rgb_overlap",
