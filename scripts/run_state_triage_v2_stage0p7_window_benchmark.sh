#!/usr/bin/env bash
# Serial dispatcher for the frozen 80-window Stage 0.7 benchmark.
set -euo pipefail
...
p = Path("docs/protocols/state-triage-v2-stage0p7-window-gate-b.json")
value = json.loads(p.read_text(encoding="utf-8"))
items = value["evaluation_inventory"]
assert len(items) == 80 and len({item["output"] for item in items}) == 80
for item in items:
    print(item["capsule_path"] + "\t" + item["output"])
')
[[ ${#records[@]} -eq 80 ]] || { echo "frozen protocol did not yield 80 records" >&2; exit 1; }
...
log="logs/state-triage-v2-stage0p7-window-${run_id}.log"
exec > >(tee -a "$log") 2>&1
echo "[$(date -Is)] Stage0.7 80-window benchmark ${run_id} start on physical GPU ${gpu_index}"
nvidia-smi --query-gpu=index,name,memory.used,memory.free,utilization.gpu --format=csv,noheader,nounits -i "$gpu_index"
index=0
for record in "${records[@]}"; do
    index=$((index + 1))
    IFS=$'\t' read -r capsule output <<< "$record"
    echo "[$(date -Is)] START ${index}/80 ${output}"
    ...
    echo "[$(date -Is)] DONE ${index}/80 ${output}"
done
touch "logs/state-triage-v2-stage0p7-window-${run_id}.done"
echo "[$(date -Is)] ALL_DONE 80/80"
