#!/usr/bin/env python3
"""Run the preregistered v9 spatial-pooled pose-export mechanism.

The program is deliberately separate from all earlier recovery exporters.  It
keeps raw predictions for Detector-v3 and replaces an alarm-frame pose only
after structural rollback, using the current frame's captured pooled latent.
"""
...
smoke = v2.smoke
SCHEMA_VERSION = "stateguard3r.recal3r-early-spatial-pooled-pose-export-v9.v1"
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--state-policy", choices=("always-commit", "detector-v3-incremental-early-spatial-pooled-pose-export"), required=True)
...
def _validate_args(args: argparse.Namespace, parser: argparse.ArgumentParser) -> None:
    if args.health_profile != "v3" or args.watchdog != 8:
        parser.error("v9 spatial pooled pose requires fixed v3 health and watchdog=8")
    if args.recovery_quality_commitment is not None:
        parser.error("v9 development may not consume a recovery-quality commitment")
    if args.input_manifest is None:
        parser.error("v9 spatial pooled pose requires a development input manifest")
    ...
    if manifest not in {path.resolve(strict=True) for path in DEVELOPMENT_MANIFESTS}:
        parser.error("v9 spatial pooled pose accepts only disclosed development manifests")
    ...
    if not isinstance(payload, dict) or payload.get("schema_version") != "stateguard3r.corruption.v1" or payload.get("source_is_read_only") is not True:
        parser.error("v9 spatial pooled pose requires a read-only development manifest")
    if args.detector_config.resolve(strict=False) != FORMAL_V3_CONFIG.resolve(strict=False):
        parser.error("v9 spatial pooled pose detector config is fixed")
...
def _self_provenance() -> dict[str, Any]:
    try:
        ...
        raise RuntimeError(f"cannot inspect v9 runner provenance: {error}") from error
    if changes:
        raise RuntimeError("StateGuard3R has tracked changes; v9 runner refuses execution")
    for component in V9_COMPONENTS:
        try:
            ...
            raise RuntimeError(f"v9 component is not tracked at HEAD: {component}") from error
    return {"repository_root": str(ROOT), "commit": commit, "tracked_worktree_clean": True, "script_path": str(Path(__file__).resolve()), "script_sha256": smoke._sha256(Path(__file__).resolve()), "python_executable": sys.executable}


def _component_provenance() -> list[dict[str, str]]:
    return [{"path": str(path.resolve(strict=True)), "sha256": smoke._sha256(path.resolve(strict=True))} for path in V9_COMPONENTS]


def _validate_new_output_dir(requested: Path) -> None:
    if requested.exists() or requested.parent != ROOT / "outputs":
        raise RuntimeError("v9 spatial pooled pose output must be a new direct outputs child")
...
def _pinned_pose_head_interface(model: Any, dpt_head_module: Any) -> tuple[Any, Any]:
    ...
    if expected is None or type(downstream) is not expected:
        raise RuntimeError("v9 model downstream head is not pinned DPTPts3dPose")
    ...
    if not callable(pose_head) or not isinstance(pose_mode, tuple) or len(pose_mode) != 3:
        raise RuntimeError("v9 pinned DPT pose-head interface is unavailable")
...
def _pose_gpu_digest(value: Any, *, torch: Any) -> str:
    ...
    if not bool(tensor.isfinite().all()):
        raise RuntimeError("v9 pose audit tensor is nonfinite")
...
class _Observation:
    ...
    decision: Any
    raw_pose: Any
    early_spatial_pooled_pose_token: Any


class _V9Observer:
    """Observe raw current predictions and perform a token-only alarm export."""
    ...
    def __init__(self, detector: Any, exporter: Any, *, timestamps: Sequence[Any], torch: Any, pose_encoding_to_camera: Any, source_hashes: Mapping[str, str]) -> None:
        self._detector, self._exporter, self._timestamps, self._torch, self._decode = detector, exporter, list(timestamps), torch, pose_encoding_to_camera
        self._source_hashes, self._previous_safe_camera = dict(source_hashes), None
        self.records: list[Any] = []
        self.observed_ledger: list[Any] = []

    def observe(self, frame_id: int, prediction: Mapping[str, Any], model: Any, frame_context: Mapping[str, Any], early_spatial_pooled_pose_token: Any) -> _Observation:
        ...
        if len(records) != frame_id + 1:
            raise RuntimeError("v9 current health trace is not prefix aligned")
        ...
        if (frame_id == 0 and overlap is not None) or (frame_id and (not isinstance(overlap, float) or not math.isfinite(overlap) or not 0.0 <= overlap <= 1.0)):
            raise RuntimeError("v9 overlap is not a valid causal current/previous value")
        ...
        errors, scales = self._torch.linalg.vector_norm(transformed - prediction["pts3d_in_other_view"], dim=-1), self._torch.linalg.vector_norm(prediction["pts3d_in_other_view"], dim=-1)
        geometric = float((_median(self._torch, errors) / self._torch.clamp(_median(self._torch, scales), min=1e-8)).item())
        if early_spatial_pooled_pose_token is None:
            raise RuntimeError("v9 candidate lacks the required spatial pooled token")
        return _Observation(frame_id, replace(records[-1], overlap=overlap, pose_jump=pose_jump, geometric_residual=geometric), self._detector.observe(replace(records[-1], overlap=overlap, pose_jump=pose_jump, geometric_residual=geometric)), pose, early_spatial_pooled_pose_token)
    ...
    def finalize(self, observation: _Observation, *, quarantined: bool, prediction: Mapping[str, Any]) -> tuple[Mapping[str, Any], Mapping[str, Any]]:
        ...
        if quarantined:
            ...
            exported, action = self._exporter.export_alarm(observation.frame_id, prediction, observation.early_spatial_pooled_pose_token, token_source=TOKEN_SOURCE)
        else:
        ...
        if evidence is not None:
            evidence["source_hashes"] = self._source_hashes
        return exported, {"export_action": action.action, "early_spatial_pooled_pose_token": evidence, "raw_candidate_pose_digest": raw_digest, "exported_camera_pose_digest": _pose_gpu_digest(exported["camera_pose"], torch=self._torch)}
...
def _exported_trajectory(predictions: Sequence[Mapping[str, Any]], records: Sequence[Any], *, decode: Any) -> list[dict[str, Any]]:
    ...
    for frame_id, (prediction, record) in enumerate(zip(predictions, records, strict=True)):
        ...
        result.append({"frame_id": frame_id, "camera_pose_encoding_absT_quaR": pose.detach().cpu().numpy()[0].tolist(), "camera_to_reference": camera.tolist(), "pose_jump": None, "geometric_residual": record.geometric_residual, "geometric_residual_semantics": "raw_candidate_pointmaps_before_export_replacement"})
    return result
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    import numpy as np
    import torch
    from stateguard3r.early_spatial_pooled_pose_export_v9 import EarlySpatialPooledPoseExport
    from stateguard3r.early_spatial_pooled_pose_v9 import audit_pinned_early_spatial_pooled_pose_sources
    ...
    from stateguard3r.recal3r_early_spatial_pooled_pose_runner_v9 import audit_v9_runner_contract, run_early_spatial_pooled_pose_recurrent_lighter
    ...
    source_audit = audit_pinned_early_spatial_pooled_pose_sources(args.baseline_src / "dust3r" / "model.py", args.baseline_src / "dust3r" / "heads" / "dpt_head.py", args.baseline_src / "dust3r" / "heads" / "postprocess.py", ROOT / "src" / "stateguard3r" / "recal3r_early_spatial_pooled_pose_runner_v9.py")
    runner_audit = audit_v9_runner_contract(ROOT / "src" / "stateguard3r" / "recal3r_early_spatial_pooled_pose_runner_v9.py")
    ...
    try:
        ...
        if args.state_policy != "always-commit":
            observer = _V9Observer(IncrementalOnlinePrefixDetector(FrozenDetectorV3Config.from_mapping(config_payload), captures=captures, capture_provenance=capture_provenance), EarlySpatialPooledPoseExport(torch=torch, pose_head=pose_head, postprocess_pose=pose_postprocess.postprocess_pose, pose_mode=pose_mode, pose_encoding_to_camera=camera.pose_encoding_to_camera), timestamps=smoke._input_timestamps(args), torch=torch, pose_encoding_to_camera=camera.pose_encoding_to_camera, source_hashes={key: source_audit[key] for key in ("model_sha256", "dpt_head_sha256", "postprocess_sha256", "runner_sha256", "decoder_projection_sha256")})
        ...
        if observer is None:
            ...
            if len(health) != len(views) or len(ledger) != len(views): raise RuntimeError("v9 observed health ledger is incomplete")
            trajectory_payload = {"schema_version": "stateguard3r.trajectory.early-spatial-pooled-pose-v9", "pose_encoding": "absT_quaR", "matrix_convention": "camera_to_first_input_frame_reference", "camera_pose_semantics": "clear frames export real raw pose; alarm frames export only the current captured early-spatial mean through frozen projection/head/postprocess; detector health remains raw", "early_spatial_pooled_token_evidence_location": "state-timeline.json transactions", "frames": trajectory}
        smoke._verify_preflight_inputs(args)
