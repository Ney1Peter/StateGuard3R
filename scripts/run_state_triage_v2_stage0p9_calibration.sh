#!/usr/bin/env bash
# Serial dispatcher for the sealed Stage 0.9 hashed-tile calibration.
set -euo pipefail
...
cd "$root"
protocol="docs/protocols/state-triage-v2-stage0p9-calibration-gate-b.json"
[[ -f "$protocol" && ! -w "$protocol" ]] || { echo "missing immutable Stage 0.9 calibration protocol" >&2; exit 1; }
mkdir -p logs tmp
mapfile -t records < <(.venv/bin/python -c '
import json
from pathlib import Path
p = Path("docs/protocols/state-triage-v2-stage0p9-calibration-gate-b.json")
items = json.loads(p.read_text(encoding="utf-8"))["calibration_inventory"]
...
for record in "${records[@]}"; do
    ...
    [[ ! -e "outputs/$output" ]] || { echo "refusing overwrite/retry: outputs/$output" >&2; exit 1; }
done
...
log="logs/state-triage-v2-stage0p9-calibration-${run_id}.log"
exec > >(tee -a "$log") 2>&1
echo "[$(date -Is)] Stage0.9 calibration ${run_id} start on physical GPU ${gpu_index}"
...
touch "logs/state-triage-v2-stage0p9-calibration-${run_id}.done"
