#!/usr/bin/env bash
# Wait without reserving GPU, then release the one v12 Gate-B control only
# through its terminal-evidence dispatcher.  This script never runs a model.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd -P)"
RECAL3R_ROOT="${ROOT}/../baselines/ReCal3R"
PYTHON_BIN="${RECAL3R_ROOT}/.venv/bin/python"
DISPATCHER="${ROOT}/scripts/dispatch_recal3r_patch_embed_global_pooled_pose_v12.py"
RUN_ID="recovery-patch-embed-global-pooled-pose-v12-dynamic-always-commit-0001"
...
require_clean_tracked_worktrees() {
    if [[ -n "$(git -C "${ROOT}" status --porcelain)" ]]; then
        record "V12_WAIT_ABORT run_id=${RUN_ID} reason=dirty_stateguard at=$(date --iso-8601=seconds)"
        return 1
    fi
    if [[ -n "$(git -C "${RECAL3R_ROOT}" status --porcelain)" ]]; then
        record "V12_WAIT_ABORT run_id=${RUN_ID} reason=dirty_recal3r at=$(date --iso-8601=seconds)"
        return 1
    fi
}
...
write_one_use_command_json() {
    if [[ -e "${COMMAND_JSON}" || -L "${COMMAND_JSON}" ]]; then
        record "V12_WAIT_ABORT run_id=${RUN_ID} reason=command_json_already_exists at=$(date --iso-8601=seconds)"
        return 1
    fi
...
argv = [
    ...
    str(root / "scripts" / "run_recal3r_patch_embed_global_pooled_pose_export_v12.py"),
...
record "V12_WAIT_START run_id=${RUN_ID} min_free_mib=${MIN_FREE_MIB} at=$(date --iso-8601=seconds)"
while true; do
    if first="$(gpu_free_mib)" && [[ "${first}" =~ ^[0-9]+$ ]] && (( first >= MIN_FREE_MIB )); then
        ...
        if second="$(gpu_free_mib)" && [[ "${second}" =~ ^[0-9]+$ ]] && (( second >= MIN_FREE_MIB )); then
            record "V12_WAIT_EXTERNAL_GATE_PASS run_id=${RUN_ID} first_free_mib=${first} second_free_mib=${second} at=$(date --iso-8601=seconds)"
            require_clean_tracked_worktrees
            write_one_use_command_json
            record "V12_WAIT_DISPATCH run_id=${RUN_ID} at=$(date --iso-8601=seconds)"
            exec "${PYTHON_BIN}" "${DISPATCHER}" --run-id "${RUN_ID}" --command-json "${COMMAND_JSON}" --timeout-seconds 7200
        fi
        record "V12_WAIT_CONFIRM_REJECTED run_id=${RUN_ID} first_free_mib=${first} second_free_mib=${second:-unavailable} at=$(date --iso-8601=seconds)"
    else
        record "V12_WAIT_POLL run_id=${RUN_ID} free_mib=${first:-unavailable} at=$(date --iso-8601=seconds)"
    fi
    unset first second
    sleep "${POLL_SECONDS}"
done
