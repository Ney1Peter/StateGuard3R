...
from scripts import run_recal3r_patch_embed_global_pooled_pose_export_v12 as script
...
def _args(
    manifest: Path, *, watchdog: int = 8, detector_config: Path | None = None,
) -> SimpleNamespace:
...
def test_v12_cli_accepts_only_disclosed_read_only_manifests_and_fixed_inputs() -> None:
    ...
    with pytest.raises(SystemExit):
        script._validate_args(
            _args(next(iter(script.DEVELOPMENT_MANIFESTS)), detector_config=PROJECT_ROOT / "other.json"), parser,
        )


def test_v12_cli_requires_new_direct_output_and_its_two_fixed_policies(tmp_path: Path) -> None:
    direct = script.ROOT / "outputs" / "test-v12-direct-output"
    ...
    assert action.choices == (
        "always-commit", "detector-v3-incremental-patch-embed-global-pooled-pose-export",
    )


def test_v12_cli_components_are_distinct_and_head_interface_fails_closed(
    ...
    with pytest.raises(RuntimeError, match="not pinned"):
        script._pinned_pose_head_interface(
            SimpleNamespace(downstream_head=object()), SimpleNamespace(DPTPts3dPose=_Pinned),
        )

    component = Path(script.__file__).resolve()
    monkeypatch.setattr(script, "V12_COMPONENTS", (component,))
...
def test_v12_observer_uses_raw_prediction_only_for_health_before_token_only_export() -> None:
    observe_source = inspect.getsource(script._V12Observer.observe)
    finalize_source = inspect.getsource(script._V12Observer.finalize)
    observe_tree = ast.parse(textwrap.dedent(observe_source))
    finalize_tree = ast.parse(textwrap.dedent(finalize_source))
    assert any(
        isinstance(node, ast.Subscript)
        and isinstance(node.value, ast.Name)
        and node.value.id == "prediction"
        and isinstance(node.ctx, ast.Load)
        for node in ast.walk(observe_tree)
    )
    assert "export_alarm(observation.frame_id, prediction, observation.capture" in finalize_source
    assert "TOKEN_SOURCE" in finalize_source
    assert "decode_patch_embed_global_pose_token" not in finalize_source
    assert not any(
        isinstance(node, ast.Call)
        and isinstance(node.func, ast.Attribute)
        and node.func.attr in {"cpu", "numpy", "tolist"}
        for node in ast.walk(finalize_tree)
    )


def test_v12_metadata_causality_text_excludes_gt_and_future_export_inputs() -> None:
    source = Path(script.__file__).read_text(encoding="utf-8")
    assert "alarm export consumes only current patch-embed-global token after rollback" in source
    assert "no raw camera_pose numeric input, fallback, future input or export feedback" in source
    assert script.TOKEN_SOURCE == "native_patch_embed_mean_before_encoder_blocks_and_rope"
    assert json.loads(json.dumps({"components": [str(path) for path in script.V12_COMPONENTS]}))["components"]
