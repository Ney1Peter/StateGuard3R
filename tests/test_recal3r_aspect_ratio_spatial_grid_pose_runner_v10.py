...
def test_v10_runner_contract_binds_candidate_pool_before_update_and_raw_control_path() -> None:
    audit = runner.audit_v10_runner_contract(RUNNER)
    assert audit["early_decoder_layer_index"] == 0
    assert audit["preprojection_spatial_token_count"] == 768
...
def test_v10_runner_contract_rejects_special_token_and_after_update_mutations(tmp_path: Path) -> None:
    ...
    with pytest.raises(runner.EarlySpatialGridPoseRunnerError, match="source contract"):
        runner.audit_v10_runner_contract(special)
    ...
    source = _replace_last(original, capture, "early_spatial_grid_pose_token = None")
    ...
    with pytest.raises(runner.EarlySpatialGridPoseRunnerError, match="source contract|after rollout"):
        runner.audit_v10_runner_contract(moved)


def test_v10_runner_candidate_pools_before_memory_mutation_and_control_avoids_projection() -> None:
    torch = _torch()
    ...
    class _Model:
        ...
        def _encode_image(self, _images: object, _shapes: object) -> tuple[list[object], object]:
            return [torch.zeros((1, 768, 1024))], torch.zeros((1, 768, 2))
        ...
        def _recurrent_rollout(self, state: object, *_args: object, **_kwargs: object) -> tuple[object, list[object], object, object, object, object]:
            self.dec0 = torch.arange(768 * 1024, dtype=torch.float32).reshape((1, 768, 1024)) / 1024.0
    ...
    candidate = runner.run_early_spatial_grid_pose_recurrent_lighter(
    ...
    expected_input = torch.arange(768 * 1024, dtype=torch.float32).reshape((1, 768, 1024)) / 1024.0
    ...
    control = runner.run_early_spatial_grid_pose_recurrent_lighter(
